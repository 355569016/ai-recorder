...
const pages = [
  ...
  },
  {
    ...
    fields: [["hotspot_ssid", "设备热点", "设备广播"], ["update_transport", "更新方式", "硬件能力"], ["system_join_preview", "系统加入弹窗示意", "本地演示"]],
    rules: ["如果硬件不支持 Wi-Fi 更新，本页不展示，直接使用蓝牙更新。", "页面中的取消和加入仅为系统操作示意，不可点击。", "点击继续后直接进入 APP-DEV-08 固件更新中页，从下载固件包开始演示，不在本页弹出系统加入确认。", "实际系统加入热点弹窗在 APP-DEV-08 的连接到 AI Recorder A1 步骤出现。"],
    states: ["等待继续", "进入固件更新", "取消更新"],
    deps: ["iOS / Android Wi-Fi 热点连接能力。", "硬件热点协议待确认。"],
    acceptance: ["页面解释手机将连接 AI Recorder A1 热点完成更新，示意卡不可操作。", "点击继续直接进入 APP-DEV-08 并显示下载固件包步骤，不弹出加入热点确认。", "页面取消返回设备详情。"]
  },
  {
    id: "APP-DEV-08",
    group: "设备",
  ...
  },
  {
    ...
    rules: ["APP-HOME-01 点击“快传”后直接进入本页，不再展示传输方式选择。", "第一版仅支持手机临时加入录音卡热点的高速传输方案，不提供让录音卡连接手机当前 Wi-Fi 的 APP-SYNC-02 流程。", "APP 内底部弹窗蒙层下复用 APP-HOME-01 文件首页，只负责解释连接设备热点的目的和联网影响。", "引导页中的加入无线局域网卡片仅为操作示意，其中取消和加入按钮不可点击。", "用户点击继续后才调用系统加入热点能力；系统原生风格的取消/加入确认弹窗直接覆盖在当前连接设备热点引导页上，不切换或复用固件更新页面。", "系统弹窗点击取消后返回等待确认；点击加入后关闭系统弹窗，回到高速传输引导页并依次展示连接中和连接成功。", "连接成功态展示 2 秒，随后自动返回文件首页并开始显示热点高速同步状态。", "连接热点期间手机可能暂时无法通过 Wi-Fi 上网；高速传输完成后恢复原网络。"],
    states: ["等待确认", "系统加入确认", "连接中", "连接成功"],
    deps: ["BLE：启动和读取录音卡 SoftAP 会话。", "iOS / Android：临时加入设备热点能力。", "设备热点鉴权和连接状态回传。"],
    acceptance: ["APP-HOME-01 点击“快传”直接进入 APP-SYNC-01，不出现传输方式选择弹层。", "第一版导航、状态链路和页面流程中不出现 APP-SYNC-02。", "APP 引导弹窗底层为 APP-HOME-01 文件首页，并展示不可点击的系统弹窗示意、继续和取消。", "点击继续后出现独立的系统加入无线局域网确认弹窗，弹窗下方仍为当前连接设备热点引导页，不出现固件更新内容。", "系统取消返回等待确认，系统加入进入连接中；连接中和连接成功状态不展示完成或取消按钮。", "成功状态停留 2 秒后自动返回文件首页，首页同步卡标识为录音卡热点。", "右侧状态链路可独立预览等待确认、连接中和连接成功。"],
    prototypeLinks: [],
...
function renderHotspotTransferContent(state) {
  ...
  return `
    ...
    <section class="firmware-sheet sync-transfer-sheet hotspot-transfer-sheet">
      <h2>连接设备热点</h2>
      <div class="sheet-line"></div>
      <p>手机将临时连接到 AI Recorder A1 的热点，以更快传输录音文件。请点击继续后，同意加入<strong>AI Recorder A1 无线局域网。</strong></p>
