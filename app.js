...
let fileDetailActiveTab = "markers";
...
const pages = [
  ...
  },
  {
    ...
    rules: ["开启同步云端时，标题优先根据云端同步数据提炼。", "未开启同步云端时，标题默认使用文件名称。", "生成 AI 会议摘要后，标题根据 AI 会议内容重新提炼总结。", "生成 AI 会议总结后，根据会议内容提取关键标签。", "播放器进度条支持点击和拖动调整音频进度。", "播放器控制区包含音频设置、快退 15 秒、播放、快进 15 秒、倍速，不展示额外方向箭头。", "播放器下方将标记作为一级内容页签，与大纲、智能总结、文字记录、待办事项和思维导图并列。", "录音中产生的文字、时间和照片标记在录音结束后完整继承到文件详情，不能依赖转写或 AI 生成状态。", "录音结束后不再允许新增标记，只能维护录音过程中已经产生的标记。", "点击标记时间或播放图标定位到对应音频时间并开始播放。", "点击标记文本、照片说明、文字记录或智能总结时，原文字直接出现光标并唤起系统键盘；不打开弹窗、新页面，不增加边框、下划线、取消或保存按钮。", "智能总结和文字记录使用系统键盘上方的轻量格式工具栏，至少包含标题/正文、列表、对齐、引用、撤销和重做；普通标记与照片说明保持纯文本键盘，不展示格式工具栏。", "编辑内容实时记录，输入框失焦、收起键盘或切换页签时自动保存。", "照片可打开和删除；标记时间与来源保持只读。", "生成 AI 内容时读取当时的标记快照作为辅助笔记；生成后继续编辑标记不会静默改写既有 AI 内容，重新生成时再读取最新标记。", "页面底部固定展示对此笔记提问输入框。", "右上角分享按钮打开分享弹窗，三个点打开更多操作浮层。"],
    ...
    acceptance: ["播放器下方必须有大纲、智能总结、文字记录、标记、待办事项、思维导图一级页签。", "录音期间的全部标记在已生成文件详情中完整展示，并标注已用于本次生成、可继续编辑。", "录音结束后的文件详情不得出现新增标记入口。", "点击标记时间定位音频；点击标记内容、文字记录和智能总结均在原文字位置直接输入，仅显示光标和系统键盘，不出现独立窗口或显式保存操作。", "总结和转写编辑时格式栏可切换标题、列表、对齐和引用；标记编辑时不显示格式栏。", "失焦、收起键盘或切换页签后内容自动保存。", "照片可查看或删除，时间与来源只读。", "思维导图按卡片预览呈现。", "音频设置和倍速按钮能打开对应弹窗。", "底部有对此笔记提问输入框。", "标题和标签规则在字段与交互说明中明确。"],
...
function renderFileDetailTabs(state = "generated") {
  ...
  return `
    <nav class="file-detail-tabs" aria-label="文件详情内容">
...
function renderFileDetail() {
  const activeTab = ["outline", "summary", "transcript", "markers", "todos", "mindmap"].includes(fileDetailActiveTab) ? fileDetailActiveTab : "markers";
  ...
  return `
    ...
    <main class="file-detail-page ${fileDetailMarkerEditor || contentEditing ? "is-editing" : ""} ${contentEditing ? "is-rich-editing" : ""}">
      ...
      ${activeTab === "markers" ? renderFileDetailMarkers("generated") : ""}
      ${activeTab === "outline" ? `<section class="ai-section outline-section" id="detail-outline">
      ...
      ${activeTab === "summary" ? `<section class="ai-section" id="detail-summary">
        <h2>智能总结</h2>
        <h3>录音信息</h3>
      ...
      ${activeTab === "transcript" ? `<section class="ai-section" id="detail-transcript">
        <h2>文字记录</h2>
      ...
      ${activeTab === "todos" ? `<section class="ai-section" id="detail-todos">
        <h2>待办事项</h2>
      ...
      ${activeTab === "mindmap" ? `<section class="ai-section mindmap-section" id="detail-mindmap">
...
function renderFileDetailEmpty() {
  ...
  return `
    ...
    <main class="file-detail-page empty-detail-page ${fileDetailMarkerEditor ? "is-editing" : ""}">
      ...
      ${activeTab === "markers" ? renderFileDetailMarkers("pending") : `<section class="empty-note-state">
        <div class="empty-note-icon"></div>
        <h2>可生成笔记</h2>
        <p>生成后将在此处显示总结，并参考已保存的 ${recordingMarks.length} 条标记。</p>
      </section>`}
    </main>
...
document.addEventListener("click", (event) => {
  ...
  if (action && action.dataset.action === "file-detail-set-tab") {
    ...
    fileDetailActiveTab = action.dataset.value || "markers";
    render();
    return;
  }
