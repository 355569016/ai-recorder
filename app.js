...
const recordingPhotoAsset = "./assets/recording-marker-notes.png";
const recordingBaseMarks = [
  { id: "recording-mark-device", time: "00:12:08", type: "mark", text: "重点标记", source: "录音卡" },
  { id: "recording-mark-photo", time: "00:27:46", type: "photo", text: "", caption: "", captionSource: "", source: "APP", photo: recordingPhotoAsset },
  { id: "recording-mark-text", time: "00:30:51", type: "input", text: "客户希望首版支持离线转写", source: "APP" }
];
let recordingMarks = recordingBaseMarks.map((mark) => ({ ...mark }));
let recordingCameraPrompt = false;
let recordingCameraOpen = false;
let recordingPhotoViewerId = "";
let recordingEditor = null;
let recordingMarkSerial = recordingBaseMarks.length;
let recordingExpandedMarkIds = new Set();
let recordingNotice = "";
let recordingNoticeTimer = null;
let fileDetailActiveTab = "summary";
let fileDetailSummaryTab = "summary";
let searchDraft = "";
let activeSearchQuery = "";
let activeSearchResult = null;
let fileDetailAudioSeconds = 0;
let fileDetailAudioPlaying = false;
let fileDetailMarkerEditor = null;
let fileDetailContentEditor = null;
let fileDetailFormatPanel = "main";
let fileDetailFormatState = {
  block: "body",
  bold: false,
  italic: false,
  underline: false,
...
let fileDetailContentFormats = {};
let fileDetailEditHistory = [];
let fileDetailEditRedo = [];
let fileDetailDockInteraction = false;
let fileDetailContent = {
  summary: "本次会议围绕拼团营销工具的需求评审展开，重点确认了功能范围、简化方案、标准商城与积分商城的拼团功能 MVP，以及后续大商品库项目的启动安排。",
  transcript: [
    {
      time: "00:00:03",
      speaker: "说话人 1",
      text: "我们先确认这次评审的目标，主要聚焦拼团营销工具的 MVP 范围。"
    },
    {
      time: "00:01:18",
      speaker: "说话人 2",
...
let fileDetailPhotoViewerId = "";
let fileDetailGeneratingInitialized = false;
let fileDetailNotice = "";
let fileDetailNoticeTimer = null;
...
let activeExportFormatMenu = "";
let exportNotice = "";
let exportSheetState = {
  audio: { format: "MP3" },
  transcript: { format: "TXT", includeTimestamps: false, includeSpeakers: false },
  marks: { format: "DOCX", includeTimestamps: true },
...
const defaultRecycleBinItems = [
  { id: "recycle-1", name: "周一项目复盘", meta: "今天 16:11 · 00:07", source: "硬件录音", deletedAt: "今天 16:18" },
  { id: "recycle-2", name: "客户访谈", meta: "07-04 18:44 · 00:28:14", source: "硬件录音", deletedAt: "昨天 21:05" },
  { id: "recycle-3", name: "Launch checklist", meta: "07-01 12:41 · 00:19:08", source: "导入音频", deletedAt: "07-02 09:30" }
];
let recycleBinItems = defaultRecycleBinItems.map((item) => ({ ...item }));
let recycleSelectionMode = false;
let selectedRecycleIds = new Set();
let recycleDeleteConfirmOpen = false;
let recycleNotice = "";
let loggedInDevices = [
...
const pages = [
  ...
  },
  {
    ...
    rules: ["绑定前必须登录。", "设备页展示设备名、完整 SN、帮助提示和设备图。", "点击 SN 后面的提示图标，弹窗说明：检查录音卡装置随附的包装盒，SN 印在包装盒背面的标签上。", "用户确认后执行连接和绑定写入。", "不是用户设备时可返回重新搜索。"],
    states: ["等待连接", "SN 位置提示", "连接中", "绑定成功", "绑定失败"],
    deps: ["BLE 命令包：写入绑定关系，协议待固件定义。", "云端设备：绑定/解绑接口待后端定义。"],
    acceptance: ["搜索到设备页展示连接按钮。", "点击 SN 后提示图标可打开 SN 位置说明弹窗。", "点击连接成功进入文件首页。", "不是用户设备可返回搜索。"]
  },
  {
    id: "APP-DEV-04",
    group: "设备",
  ...
  },
  {
    ...
    prototypeLinks: [["立即更新", "APP-DEV-07"]],
    prototypeActions: [["蓝牙更新", "start-firmware-update", "bluetooth"]]
  },
  {
    id: "APP-DEV-07",
    group: "设备",
    ...
    fields: [["hotspot_ssid", "设备热点", "设备广播"], ["update_transport", "更新方式", "硬件能力"], ["system_join_instruction", "系统加入操作说明", "本地文案"]],
    rules: ["如果硬件不支持 Wi-Fi 更新，本页不展示，直接使用蓝牙更新。", "页面只说明连接设备热点的目的、网络影响和后续操作，不展示可点击感强的系统弹窗示意。", "点击继续更新后直接进入 APP-DEV-08 固件更新中页，从下载固件包开始演示，不在本页弹出系统加入确认。", "实际系统加入热点弹窗仍在 APP-DEV-08 的连接到 AI Recorder A1 步骤出现。"],
    states: ["等待继续", "进入固件更新", "取消更新"],
    deps: ["iOS / Android Wi-Fi 热点连接能力。", "硬件热点协议待确认。"],
    acceptance: ["页面使用两步说明解释手机将连接 AI Recorder A1 热点传输固件包，并提示系统弹窗出现后选择加入。", "页面不展示模拟系统弹窗，也不展示等待确认状态卡。", "点击继续更新直接进入 APP-DEV-08 并显示下载固件包步骤，不弹出加入热点确认。", "页面取消返回设备详情。"]
  },
  {
    id: "APP-DEV-08",
    group: "设备",
    ...
    entry: "连接设备热点页点击继续；或选择蓝牙更新。",
    fields: [["firmware_package", "固件包", "云端固件服务"], ["update_step", "更新步骤", "固件更新服务"], ["update_progress", "更新进度", "本地任务状态"], ["system_join_prompt", "系统加入热点弹窗", "系统权限"], ["device_safe_state", "设备安全状态", "BLE 状态包"]],
    rules: ["进入本页先显示下载固件包，加载 3 秒后自动进入连接到 AI Recorder A1。", "Wi-Fi 更新进入连接步骤时自动弹出系统加入热点确认；点击加入后立即开始传输固件包，传输 3 秒后进入安装固件包。", "系统弹窗点击取消后停留在连接到 AI Recorder A1，并显示重新连接按钮；点击后再次唤起加入热点弹窗。", "进入安装固件包后停留在当前状态，不自动跳转更新完成；通过右侧状态链路的更新完成按钮进入文件首页。", "蓝牙更新不弹出热点确认，连接、传输和安装过程由页面自动推进。", "离开本页时停止未完成的原型计时，避免后台继续推进。"],
    states: ["下载固件包", "连接到 AI Recorder A1", "等待重新连接", "传输固件包到 AI Recorder A1", "安装固件包", "更新完成", "连接失败"],
    deps: ["云端固件包下载。", "BLE / Wi-Fi 传输协议。", "设备安装进度回传。"],
    acceptance: ["进入页面时下载固件包为当前步骤，3 秒后自动进入连接步骤并在 Wi-Fi 更新时弹出加入热点确认。", "点击取消停留在连接步骤并显示重新连接；点击重新连接可再次弹出确认。", "点击加入后传输步骤持续 3 秒，随后进入安装固件包并保持停留。", "右侧连接失败进入失败页，更新完成进入 APP-HOME-01 文件首页。"],
    prototypeLinks: [["连接失败", "APP-DEV-09"], ["更新完成", "APP-HOME-01"]]
  },
  {
    id: "APP-DEV-09",
    group: "设备",
    ...
    rules: ["热点连接失败时提示靠近设备或改用蓝牙更新。", "点击重试后返回 APP-DEV-08 固件更新中页，从连接到 AI Recorder A1 步骤继续更新；Wi-Fi 更新重新唤起加入热点确认。", "蓝牙更新失败时点击重试返回固件更新中页，并从连接步骤继续。", "改用蓝牙更新后回到固件更新中页并切换更新方式。"],
    states: ["热点失败", "蓝牙失败", "重试", "改用蓝牙更新"],
    deps: ["系统 Wi-Fi 连接状态。", "BLE 连接状态。"],
    acceptance: ["失败弹窗包含重试和改用蓝牙更新按钮。", "重试进入 APP-DEV-08 并从连接步骤继续更新，不返回 APP-DEV-07 说明页。", "改用蓝牙更新进入 APP-DEV-08 并切换为蓝牙更新。"],
    prototypeActions: [["重试", "retry-firmware-update"], ["改用蓝牙更新", "start-firmware-update", "bluetooth"]]
  },
  {
    id: "APP-SYNC-01",
    group: "同步",
    ...
    fields: [["device_hotspot_ssid", "录音卡热点名称", "BLE/设备热点"], ["hotspot_join_state", "手机加入热点状态", "系统 Wi-Fi"], ["system_join_prompt", "系统加入无线局域网确认", "操作系统"], ["personal_hotspot_conflict_tip", "个人热点冲突提示", "本地文案"], ["hotspot_connection_step", "热点连接步骤", "设备热点/系统 Wi-Fi"], ["network_impact_tip", "联网影响提示", "本地文案"], ["fast_transfer_saved_minutes", "预计节省等待时间", "同步队列/传输速率估算"]],
    rules: ["APP-HOME-01 点击“快传”后直接进入本页，不再展示传输方式选择。", "第一版仅支持手机临时加入录音卡热点的高速传输方案，不提供让录音卡连接手机当前 Wi-Fi 的 APP-SYNC-02 流程。", "APP 内底部弹窗蒙层下复用 APP-HOME-01 文件首页，只负责解释连接设备热点的目的、下一步系统操作和联网影响。", "连接前说明页不展示模拟系统弹窗或等待确认状态卡，只提示系统弹窗出现后选择加入。", "用户点击继续并连接后才调用系统加入热点能力；系统原生风格的取消/加入确认弹窗直接覆盖在当前连接设备引导页上，不切换或复用固件更新页面。", "系统弹窗点击取消后返回连接前说明；点击加入后关闭系统弹窗，进入底部连接状态面板。", "连接状态面板顶部常驻提示“若已开启个人热点，你将无法连接到该设备的 Wi-Fi。请关闭后重试。”，用于在连接失败前提示个人热点冲突。", "连接中按顺序展示两步：第一步“AI Recorder A1 的 Wi-Fi 已打开”为完成态；第二步“连接到 AI Recorder A1 的热点”为进行态。页面不再展示独立大号加载圆环。", "连接中底部仅保留取消；取消后中止本次高速连接并返回文件首页，继续使用蓝牙同步。", "连接失败时保留第一步完成态，第二步切换为失败态，底部提供重试和取消；重试只重新执行热点连接，不重复开启设备 Wi-Fi。", "连接成功后立即返回文件首页，以“已连接，开始高速传输”短提示和文件传输进度承接，不展示独立成功页或额外等待。", "Wi-Fi 快传中保留本批次整体进度、百分比、已完成数/总数和预计时间，并展示“加速快传中”与预计节省等待时间。", "点击“结束快传”后断开设备 Wi-Fi，从当前文件偏移和批次进度继续使用蓝牙传输，不重新创建批次或清零进度。", "连接热点期间手机可能暂时无法通过 Wi-Fi 上网；传输结束后恢复原网络。"],
    states: ["连接前说明", "系统加入确认", "连接中（两步状态）", "连接失败（可重试）", "连接成功并开始传输（首页提示）", "Wi-Fi 快传中", "结束快传并由蓝牙续传"],
    deps: ["BLE：启动和读取录音卡 SoftAP 会话。", "iOS / Android：临时加入设备热点能力。", "设备热点鉴权和连接状态回传。"],
    acceptance: ["APP-HOME-01 点击“快传”直接进入 APP-SYNC-01，不出现传输方式选择弹层。", "第一版导航、状态链路和页面流程中不出现 APP-SYNC-02。", "APP 引导弹窗底层为 APP-HOME-01 文件首页，并使用两步说明提示下一步在系统弹窗中选择加入。", "连接前说明不展示模拟系统弹窗或等待确认状态卡，只保留继续并连接和暂不使用。", "点击继续并连接后出现独立的系统加入无线局域网确认弹窗，弹窗下方仍为当前录音文件传输引导页，不出现固件更新内容。", "系统取消返回连接前说明；系统加入后展示底部连接状态面板，文件首页及顶部内容全部处于同一蒙版下。", "连接状态面板展示个人热点冲突提示、已完成的设备 Wi-Fi 开启步骤和正在进行的热点连接步骤，不展示独立大号加载圆环。", "连接失败后可重试或取消；取消回到文件首页并继续蓝牙同步，重试时保留设备 Wi-Fi 已打开状态。", "连接成功后立即进入文件首页，不展示独立成功页；首页短提示为“已连接，开始高速传输”，文件列表显示正在传输及进度。", "Wi-Fi 快传状态显示完整进度条、Wi-Fi 快传方式、结束快传入口，以及“加速快传中 / 预计节省约 X 分钟”。", "结束快传后继续使用蓝牙传输，整体百分比、已完成数、当前任务和队列顺序均保持不变。", "右侧状态链路可独立预览连接前说明、连接中、连接失败、成功后开始传输和结束快传后蓝牙续传。"],
    prototypeLinks: [],
    prototypeActions: [["连接前说明", "set-hotspot-state", "awaiting"], ["连接中", "set-hotspot-state", "connecting"], ["连接失败", "set-hotspot-state", "failed"], ["开始传输", "set-hotspot-state", "success"]]
  },
  {
    id: "APP-HOME-01",
    group: "文件",
    ...
    rules: [
      ...
      "用户结束录音后，首页才展示结束录音后的同步传送状态，将录音期间尚未完成的分片继续补传、校验并合并。",
      "结束录音后立即在全部文件顶部建立临时文件记录；文件等待传输或正在传输时保留该记录，补传完成后转为正常文件并显示未转写。",
      "首页左上设备状态使用紧凑设备图标入口，不再使用大面积状态卡。",
      "账号尚未添加录音卡时，设备摘要位置复用已添加状态的图标区与信息区布局：左侧使用中性的录音卡入口视觉，右侧显示“添加设备”和轻量引导，不使用贯穿整行的大描边按钮，也不展示设备名称、连接状态、电量、存储或设备下拉入口；点击整行进入 APP-DEV-01 正在搜索设备。",
      "未添加录音卡状态不展示同步状态条、快传入口，以及正在传输或等待传输的置灰文件记录；已同步完成和导入 App 的正常文件仍可浏览。",
      "当前已选择录音卡的名称、连接状态、电量和存储需要直接显示在设备入口外层，不必展开下拉即可查看。",
      "点击设备图标进入当前设备详情；点击设备旁下拉箭头展开添加设备和已绑定设备列表。",
      "蓝牙系统层面可支持手机同时连接多个 BLE 外设，但 AI 录音卡是否允许多设备并发录音/同步取决于固件连接数、带宽、同步队列和冲突处理；MVP 建议支持多设备绑定与切换，当前活跃录音/同步设备一次只选一个。",
      ...
      "等待传输状态只显示“等待传输”，不显示队列顺位、传输方式、已传输/总文件大小或单文件进度。",
      "等待传输任务不展示常驻“优先同步”按钮；点击或长按“等待传输”状态打开轻量顺序浮层，可将非下一项任务设为优先。",
      "全部文件中的正在传输和等待传输行整体置灰，且不显示右箭头或进入详情操作；正在传输行不可点击并可显示单文件百分比，等待传输行仅状态区域可点击或长按。",
      "同步传输失败后优先进入后台自动重试；设备恢复连接时自动续传和重试。未达到自动重试限额期间，文件维持置灰的正在重试状态，不显示失败状态或手动重试按钮；达到限额后才显示轻量“同步失败 · 重试”操作，且不显示“录音已安全保留”等未经确认的保障文案。",
      ...
      "只有达到自动重试限额的同步失败文件在右侧保留轻量“同步失败 · 重试”操作，其他状态不占用独立操作列。",
      "点击导入音频打开导入音频弹层，可从文件或相册调起系统选择器。",
      "底部中间录音按钮是全局主按钮，不是普通 Tab。"
    ],
    criticalRules: [
    ...
    acceptance: [
      ...
      "正在传输和等待传输文件整体置灰，不显示右箭头、详情操作或单文件进度条；正在传输文件显示“正在传输… + 单文件百分比”，等待文件只显示“等待传输”。",
      "等待传输文件不显示队列顺位、传输方式或已传输/总文件大小。",
      "同步传输失败且未达到自动重试限额时，不显示失败状态或重试按钮；设备重新连接后必须自动继续重试。达到自动重试限额后，文件才显示轻量“同步失败 · 重试”，不显示“录音已安全保留”；点击重试后该行切换为置灰的正在重试状态。",
      "同步完成后文件恢复正常状态，不展示同步完成标签；本批次全部完成后同步状态条消失。",
      "当前无同步任务时，顶部同步状态、快传入口和置灰传输文件均不可见，全部文件从正常文件开始展示。",
      "未完成同步任务少于 2 个且预计剩余时间少于 3 分钟时，顶部同步状态条及其整体进度和快传入口全部隐藏；两个条件必须同时满足。",
      "等待传输文件默认只显示“等待传输”，不显示“优先同步”或其他右侧按钮；点击或长按状态打开同一轻量浮层，非下一项可设为优先，当前下一项只提示“当前已是下一项”。",
      "设置优先后，目标任务移动到当前传输文件之后，状态改为“已优先”，底部 Toast 短暂提示“已设为优先，将在当前文件后传输”；浮层不推动列表布局并可点击空白处或等待 4 秒关闭。",
      "单个任务完成后已完成数增加并开始下一项；新批次开始后文件列表不展示上一批次的同步队列历史。",
      "首页设备状态为紧凑图标入口，点击设备图标进入当前设备详情。",
      "未添加录音卡时，原设备摘要区域以与已添加状态同尺寸、同分区的无外框入口显示“添加设备”；不展示设备名称、电量、存储或连接状态，点击整行进入 APP-DEV-01，且页面不显示同步状态、快传入口和置灰的正在传输/等待传输文件。",
      "当前已选择录音卡的名称、连接状态、电量、存储在设备入口外层可直接查看。",
      "点击设备状态旁下拉箭头展开添加设备和多个已绑定设备。",
      "已绑定设备示例至少包含两个已连接设备和一个已断开设备，点击设备行可以切换当前设备。",
      "标题显示为全部文件，倒三角筛选/排序入口与导入音频按钮位于同一标题栏。",
      "正常文件行不显示生成、详情或右箭头，整行点击进入对应详情状态；未转写文件进入 APP-FILE-10 并保留详情页内生成入口，AI 已生成文件进入 APP-FILE-01。",
      "仅达到自动重试限额的同步失败行显示轻量“同步失败 · 重试”，点击后进入正在重试状态。",
      "导入音频弹层包含从文件里导入和从相册里导入两个系统选择入口。",
      "下拉/继续滚动可加载更多文件。",
  ...
  },
  {
    ...
    entry: "文件列表标题栏倒三角按钮。",
    prototypeLinks: [["回收站", "APP-FILE-23"]],
    fields: [["sort_field", "排序字段", "本地筛选"], ["folder_id", "文件夹", "本地数据库"], ["folder_name", "文件夹名称", "用户输入"], ["folder_file_count", "文件夹文件数", "本地数据库"], ["source_type", "来源类型（hardware_recording / imported_audio）", "本地数据库"], ["source_device_id", "来源设备 ID", "绑定设备记录 / 文件元数据"], ["source_device_name", "来源设备名称", "绑定设备记录"], ["trash_count", "回收站文件数", "本地数据库"]],
    rules: ["筛选是用户可见操作，作为 App 内弹层展示。", "默认按创建时间排序。", "全部文件、未分类、回收站为固定入口。", "点击回收站进入独立回收站页面，不在筛选弹层内展开回收站内容。", "“来自”表示文件的实际来源：每一台绑定设备按稳定 device_id 独立成项，显示该设备当前名称，例如 AI Recorder A1、AI Recorder A1 Pro；导入音频不关联设备，统一显示为“导入”。", "同型号的多台绑定设备不得仅按型号合并；应按 device_id 区分，并使用设备昵称或序列号后四位消歧。", "每个来源项后的数量仅统计当前文件范围内来自该设备或导入的文件。", "文件夹标题右侧加号用于新增文件夹，点击后展示文件夹命名输入框，保存后新文件夹加入文件夹列表。", "每个自定义文件夹右侧三个点打开操作菜单，支持重命名和删除文件夹。", "删除空文件夹可直接删除；删除非空文件夹时必须二次确认，并提示将同步删除文件夹内的内容。"],
    states: ["默认", "新增文件夹", "重命名文件夹", "删除空文件夹", "删除非空文件夹确认", "已选择文件夹", "已选择来源", "清空筛选"],
    deps: ["本地数据库：文件夹、来源、删除状态、设备来源。"],
    acceptance: ["文件首页点击筛选按钮打开筛选和排序弹层。", "弹层为底部筛选和排序面板，包含创建时间、文件夹和“来自”筛选。", "点击回收站进入 APP-FILE-23 回收站页面。", "“来自”逐项显示有文件的绑定设备名称，并单独显示“导入”；选择某台设备后只保留由该 device_id 产生的文件，选择“导入”后只保留导入音频。", "点击文件夹加号可输入名称并新增文件夹。", "点击文件夹三个点可重命名或删除文件夹。", "非空文件夹删除前必须出现确认弹窗，并明确同步删除文件夹内容。"]
  },
  {
    id: "APP-FILE-23",
    group: "文件",
    title: "回收站",
    priority: "P0",
    goal: "查看已移入回收站的音频，并通过单选或多选恢复文件或永久删除文件。",
    entry: "APP-FILE-03 筛选和排序弹层点击回收站。",
    prototypeActions: [["恢复列表", "show-recycle-list"], ["空状态", "show-recycle-empty"]],
    fields: [["trash_file_list", "回收站文件列表", "本地数据库"], ["selected_file_ids", "已选文件 ID", "用户选择"], ["selected_count", "已选数量", "本地状态"], ["trash_action", "恢复 / 删除", "用户操作"], ["trash_retention_days", "保留天数", "产品规则"]],
    rules: ["回收站文件保留 30 天，之后自动永久删除。", "默认展示文件名、录音时间与时长、来源和移入回收站时间。", "点击任意文件行即进入选择模式并勾选该文件，继续点击其他文件可多选。", "选择模式顶部显示已选择数量和完成；再次点击已选文件可取消勾选。", "只有至少选中一个文件时，底部才显示恢复和删除操作。", "恢复后文件回到全部文件列表并从回收站移除。", "删除表示永久删除，执行前必须二次确认并明确此操作无法撤销。", "回收站为空时显示暂无音频记录，不显示选择按钮和批量操作栏。"],
    states: ["列表", "单选", "多选", "删除确认", "恢复完成", "永久删除完成", "空状态"],
    deps: ["本地数据库：文件删除状态、删除时间和原文件夹。", "文件索引与云端对象删除能力。"],
    acceptance: ["APP-FILE-03 点击回收站进入本页。", "点击文件行可进入单选并继续多选，顶部已选数量随选择变化。", "未选文件时不显示恢复和删除操作；有选中项时底部操作栏出现。", "点击恢复后所选文件从回收站移除，并提示已恢复。", "点击删除先出现不可撤销确认，确认后才永久移除文件。", "所有文件被恢复或删除后显示暂无音频记录。"]
  },
  {
    id: "APP-FILE-13",
    group: "文件",
  ...
  },
  {
    ...
    title: "准备录音",
    priority: "P0",
    goal: "点击底部中间录音按钮后确认自动识别策略，并可选录音场景后开始录音。",
    entry: "文件首页或任意底部导航页面点击录音主按钮。",
    prototypeActions: [["自动识别", "select-record-mode", "auto"], ["指定场景", "select-record-mode", "manual"]],
    prototypeLinks: [["录音中", "APP-REC-04"]],
    fields: [["record_mode", "自动识别/指定场景", "用户选择"], ["record_scene", "指定录音场景", "用户选择/上次选择"], ["device_state", "设备可录音状态", "BLE 状态包"]],
    rules: ["进入弹层前已完成登录、绑定、设备连接、电量、存储检查。", "自动识别场景与选择录音场景互斥，默认选择自动识别场景。", "选择录音场景后直接在当前弹层展示会议、访谈、课堂和电话，不进入二级页面。", "首次选择录音场景默认勾选会议；切回自动识别后再次选择录音场景，继续勾选上一次的具体场景。", "只有点击开始录音按钮才向硬件发送开始录音命令。"],
    states: ["自动识别场景", "指定录音场景", "会议", "访谈", "课堂", "电话", "设备未连接", "低电量", "存储不足"],
    deps: ["BLE 命令包：开始录音，协议待固件定义。", "AI 场景识别服务或本地场景配置。"],
    acceptance: ["弹层展示当前连接的录音设备。", "两种场景选择方式只能勾选一个。", "指定场景在当前弹层展开且不包含自动选项。", "重新选择指定场景时恢复上次选择。", "录音面板和遮罩完整覆盖文件列表状态，不出现等待传输文字穿透。", "点击开始录音后进入录音中页。"]
  },
  {
    id: "APP-REC-03",
    group: "录音",
    title: "选择模板",
    priority: "P0",
    goal: "在转写与生成前从模板库选择转写与 AI 笔记模板。",
    entry: "转写与生成页面点击模板。",
    fields: [["template_id", "模板", "本地配置/云端模板"], ["template_category", "模板分类", "本地配置/云端模板"], ["template_output", "生成内容", "AI 模板服务"]],
    rules: ["进入页面后直接展示模板分类列表，不展示我的模板与探索标签栏。", "模板按通用、会议、演讲、通话、面试/采访等分类展示。", "本页用于自定义生成的模板选择，必须先选中模板，再点击使用此模板写回转写与生成页面。", "未选择模板时使用此模板按钮保持禁用。"],
    states: ["未选择", "已选择", "模板加载失败"],
    deps: ["AI 模板服务或本地模板配置，接口待后端定义。"],
    acceptance: ["进入页面即展示按分类组织的模板列表。", "页面不出现我的模板与探索标签栏。", "未选择模板时不能提交。", "选中模板有清晰的选中态，点击使用此模板后返回转写与生成页面并显示模板名称。"]
  },
  {
    id: "APP-REC-04",
    group: "录音",
    ...
    entry: "硬件开始录音成功后。",
    prototypeActions: [["默认", "reset-recording-demo"], ["输入态", "start-recording-input"], ["相机", "open-recording-camera"], ["AI 照片说明", "simulate-recording-photo-ai"]],
    fields: [["record_duration", "录音时长", "BLE 状态包/本地计时"], ["record_state", "录音状态", "BLE 状态包"], ["live_sync_offset", "已同步偏移量", "BLE 分片传输"], ["pending_sync_size", "待补传大小", "同步队列"], ["battery_level", "电量", "BLE 状态包"], ["storage_remaining", "存储", "BLE 状态包"], ["marker_list", "标记记录", "BLE 标记包/本地数据库"], ["marker_type", "标记类型", "输入/拍照/时间标记"], ["marker_source", "标记来源", "APP/录音卡"], ["marker_text", "标记文本", "用户输入/默认标记文本"], ["marker_assets", "标记附件", "系统相机/本地照片"], ["marker_photo_caption", "照片说明", "用户输入/AI 图片识别"], ["marker_caption_source", "照片说明来源", "empty/user/ai"], ["disconnect_timer_mode", "断连计时模式", "本地时钟"]],
    rules: ["暂停后停留录音中页。", "录音过程中 App 持续从硬件接收音频分片，实时更新首页同步队列，但正式文件需等结束录音和补传完成后生成。", "结束录音后回到文件首页，在首页展示同步传送中状态；完成 CRC/时长/标记校验后新增到文件列表。", "未进入编辑状态时，录音面板底部常驻输入、拍照、标记三个核心辅助功能按钮；键盘输入状态隐藏该功能栏，只展示文本编辑栏和系统键盘。", "点击输入先进入键盘输入状态；点击完成后才新增输入标记，并记录当前录音时间戳、来源 APP 与文本内容。", "点击标记立即新增时间标记，默认记录当前录音时间、来源 APP 与默认文本重点标记；录音卡短按标记记录来源录音卡。", "普通标记直接展示文本，不显示右侧箭头；仅文本超长时折叠并提供展开/收起。", "点击输入标记或时间标记文本进入编辑；时间与来源为只读锚点，时间标记支持删除文本但保留时间锚点。", "点击拍照先唤起系统相机；拍照完成后新增照片标记，记录当前录音时间、来源 APP 和照片附件。", "照片标记展示可点击的小图预览，点击照片打开大图；右上角删除图标删除整条照片标记。", "照片下方说明默认空，点击可编辑；转写完成后若说明仍为空且用户未编辑，AI 可结合图片与录音内容生成说明，用户编辑内容不得被 AI 自动覆盖。", "用户点击添加标记或拍照标记时，必须记录当前录音时间戳、标记类型、来源、备注或照片附件，并随音频分片一起进入同步队列。", "录音标记必须在文件详情中形成锚点映射，例如在转写记录 05:12 对应文字右侧内嵌当时拍摄的照片或备注。", "蓝牙断连时硬件仍在本地录音，App 波形图置灰，计时器继续以手机本地时钟模拟增长。", "蓝牙断连期间显示轻微呼吸红点和文案：蓝牙已断开，硬件仍安全录音中，靠近后自动恢复同步。", "蓝牙恢复后根据硬件录音状态包校准本地模拟计时、同步偏移量和标记时间戳。"],
    states: ["录音中", "暂停中", "输入标记键盘态", "编辑标记键盘态", "系统相机态", "照片标记已插入", "照片大图预览", "照片说明为空", "AI 生成照片说明", "标记文本已删除", "设备断连但仍录音", "本地模拟计时", "蓝牙恢复同步", "低电量", "存储不足"],
    deps: ["BLE 命令包：暂停、继续、结束、添加标记。", "BLE 状态包：录音状态、电量、存储。", "BLE 分片传输：实时音频片段、偏移量、校验值。", "系统相机与照片访问权限。", "本地数据库：标记时间戳、类型、来源、文本、照片附件和照片说明。", "AI 图片识别：转写后结合照片与录音内容生成说明，接口待定义。", "同步队列：结束录音后的待补传片段。", "文件详情：转写记录锚点映射。"],
    acceptance: ["计时、连接、电量、存储、标记和结束按钮清晰可见。", "点击暂停后按钮变为继续。", "未编辑时底部输入、拍照、标记三个按钮常驻可见；键盘态不得重复展示这三个页面功能按钮。", "点击输入进入可实际输入的键盘状态；点击完成后新增包含时间、来源 APP 与输入内容的标记，取消不新增。", "点击标记立即新增时间标记，并展示时间、来源 APP 和默认文本。", "录音卡标记展示来源录音卡；所有普通标记不显示右侧箭头，超长文本才出现展开/收起。", "点击输入标记和时间标记可编辑文本；时间标记可删除文本但保留时间与来源。", "点击拍照必须先表达唤起相机，拍照完成后插入带缩略图、时间和来源 APP 的照片标记。", "照片缩略图可打开大图，右上角删除图标可删除照片标记。", "照片说明默认空且可点击编辑；AI 说明只在转写完成且用户未编辑时生成，不覆盖用户内容。", "点击结束后必须回到文件首页并展示同步传送中状态，而不是直接静默生成文件。", "补传完成后首页文件列表必须新增本次录音。", "录音中的文字、时间和拍照标记必须能在文件详情转写记录中按时间戳定位展示。", "蓝牙断连时界面必须置灰波形、持续模拟计时并给出安全录音提示。"]
  },
  {
    id: "APP-FILE-01",
    group: "文件",
  ...
  },
  {
    ...
    fields: [["display_title", "展示标题", "文件名/云端同步数据"], ["audio_progress", "播放进度", "本地播放器"], ["marker_list", "录音标记列表", "本地数据库"], ["marker_generation_role", "生成辅助笔记", "AI 生成任务快照"], ["generation_state", "生成状态", "AI 服务"], ["estimated_audio_duration", "音频时长", "本地播放器"]],
    rules: ["空状态保留标题、音频播放器和右上角操作。", "未生成状态以标记作为默认一级页签，完整展示录音期间的标记；录音结束后不允许新增标记，只能原位编辑已有文字或照片说明、查看照片和删除照片。", "总结页签显示可生成笔记提示，底部固定生成主按钮。", "生成按钮文案只显示生成；点击后进入转写与生成弹层，并在确认时冻结本次标记快照。"],
    states: ["标记页签", "总结空状态", "标记原位编辑", "未生成 AI 摘要", "可生成笔记", "生成中", "生成失败"],
    deps: ["本地播放器。", "AI 服务：生成入口和状态。", "用量服务：生成前确认。"],
    acceptance: ["空详情显示标记/总结两个页签；默认展示已保存标记，并说明生成时将作为辅助笔记。", "标记可定位播放和原位编辑，但不得新增。", "总结空状态不展示已转写内容和笔记提问框。", "底部按钮只显示生成。"],
    prototypeLinks: [["文件详情已转写", "APP-FILE-01"], ["文件详情为空", "APP-FILE-10"], ["转写与生成", "APP-AI-01"], ["生成中", "APP-FILE-15"]]
  },
  {
    id: "APP-FILE-15",
    group: "文件",
    ...
    entry: "转写与生成点击立即生成。",
    fields: [["display_title", "展示标题", "文件名/云端同步数据"], ["audio_progress", "播放进度", "本地播放器"], ["marker_list", "录音标记列表", "本地数据库"], ["generation_marker_snapshot", "本次生成标记快照", "AI 生成任务"], ["generation_state", "生成状态", "AI 服务"], ["generation_hint", "生成提示", "本地文案"]],
    rules: ["生成中页面保留同一文件详情顶部、播放器以及标记/总结两个页签。", "默认打开总结页签，显示骨架占位和生成中提示；切换标记仍可查看、定位并原位编辑已有标记，但不能新增。", "本次任务使用确认生成时冻结的标记快照；生成中修改的已有标记仅用于下次生成。", "生成完成后直接进入文件详情已转写状态。", "离开页面不影响生成进度。"],
    states: ["总结生成中", "标记页签", "编辑标记", "生成完成", "生成失败"],
    deps: ["AI 服务：生成状态轮询或推送。", "本地任务状态：离开页面后继续跟踪。", "本地标记数据库与生成快照。"],
    acceptance: ["文件列表和文件详情为空点击生成都先进入转写与生成。", "生成中页结构与未生成页连续，默认打开总结并可切回标记维护。", "标记页明确说明本次生成已读取、后续修改用于下次生成。", "完成状态进入文件详情页，不再停留在生成确认页。"],
  ...
  },
  {
    ...
    entry: "文件详情右上角点击分享按钮。",
    fields: [["share_target", "分享目标", "用户选择"], ["share_link_state", "分享链接状态", "云端分享"], ["copy_content_type", "复制内容", "转写/标记/总结"], ["export_content_type", "导出内容", "录音/转写/标记/总结/思维导图"], ["export_format", "导出格式", "用户选择"], ["export_include_options", "导出附加内容", "用户选择"]],
    rules: ["分享按钮打开底部分享弹窗。", "分享链接进入分享链接设置页。", "复制到剪贴板包含复制转写、复制标记和复制总结三个选项，右侧均使用复制按钮。", "导出文件支持录音、转写、标记、总结和思维导图；转写可选择时间戳和自动标注说话人，标记可选择时间戳。", "导出总结时仅包含总结部分，不含思维导图；思维导图支持单独导出。", "所有导出项均进入对应底部弹窗，格式可选择，点击导出后给出已生成反馈。"],
    states: ["默认", "分享链接", "复制到剪贴板", "导出文件"],
    deps: ["云端分享链接。", "剪贴板权限。", "导出文件生成。"],
    acceptance: ["分享弹窗包含分享链接、复制到剪贴板和导出文件三组内容；复制到剪贴板包含转写、标记、总结三个选项，右侧不显示右转箭头。", "导出文件组按录音、转写、标记、总结、思维导图顺序展示，五项均能打开对应导出弹窗；格式、开关、关闭和导出反馈可操作。"]
  },
  {
    id: "APP-FILE-24",
    group: "文件",
    title: "导出录音",
    priority: "P0",
    goal: "选择录音文件格式并导出当前录音。",
    entry: "分享弹窗点击导出录音。",
    fields: [["export_format", "导出格式", "用户选择"], ["export_state", "导出状态", "本地任务状态"]],
    rules: ["支持 MP3 和 WAV 格式，默认 MP3。", "关闭返回分享弹窗，导出后展示已生成反馈。"],
    states: ["默认", "格式选择", "已生成"],
    deps: ["本地音频文件。", "系统文件导出/分享能力。"],
    acceptance: ["可切换 MP3/WAV 并发起导出。"]
  },
  {
    id: "APP-FILE-25",
    group: "文件",
    ...
    goal: "选择转写附加内容和格式后导出。",
    entry: "分享弹窗点击导出转写。",
    fields: [["include_timestamps", "包含时间戳", "用户选择"], ["include_speakers", "包含自动标注说话人", "用户选择"], ["export_format", "导出格式", "用户选择"]],
    rules: ["支持时间戳和自动标注说话人两个独立开关。", "支持 TXT、SRT、DOCX、PDF，默认 TXT。"],
    states: ["默认", "包含项切换", "格式选择", "已生成"],
    deps: ["转写内容和说话人信息。", "系统文件导出/分享能力。"],
    acceptance: ["两个包含项与格式均可独立切换。"]
  },
  {
    id: "APP-FILE-26",
    group: "文件",
    ...
    goal: "选择标记附加内容和格式后导出。",
    entry: "分享弹窗点击导出标记。",
    fields: [["include_timestamps", "包含时间戳", "用户选择"], ["export_format", "导出格式", "用户选择"]],
    rules: ["时间戳默认包含。", "支持 DOCX 和 PDF，默认 DOCX。"],
    states: ["默认", "时间戳切换", "格式选择", "已生成"],
    deps: ["录音标记和时间戳。", "系统文件导出/分享能力。"],
    acceptance: ["可切换时间戳并以 DOCX/PDF 导出标记。"]
  },
  {
    id: "APP-FILE-27",
    group: "文件",
    ...
    goal: "选择总结格式并只导出总结内容。",
    entry: "分享弹窗点击导出总结。",
    fields: [["export_content", "导出内容", "固定为总结"], ["export_format", "导出格式", "用户选择"]],
    rules: ["仅导出总结，不包含思维导图。", "支持 TXT、Markdown、DOCX、PDF，默认 Markdown。"],
    states: ["默认", "格式选择", "已生成"],
    deps: ["AI 总结内容。", "系统文件导出/分享能力。"],
    acceptance: ["总结内容与思维导图单独导出，格式可选择。"]
  },
  {
    id: "APP-FILE-28",
    group: "文件",
    ...
    goal: "选择思维导图格式并单独导出。",
    entry: "分享弹窗点击导出思维导图。",
    fields: [["export_content", "导出内容", "思维导图/总结结构"], ["export_format", "导出格式", "用户选择"]],
    rules: ["思维导图可单独导出。", "支持 JPEG 和 Markdown，默认 JPEG。"],
    states: ["默认", "格式选择", "已生成"],
    deps: ["思维导图数据。", "系统文件导出/分享能力。"],
    acceptance: ["可切换 JPEG/Markdown 并发起思维导图导出。"]
  },
  {
    id: "APP-FILE-07",
    group: "文件",
  ...
  },
  {
    ...
    rules: ["进入查看权限时默认全选录音、转写、标记、总结。", "点击权限项可移除或恢复对应的录音、转写、标记、总结内容。", "点击更新后保存查看权限设置并返回已生成分享链接页。", "至少保留一项内容时更新按钮可点击；全部移除时更新按钮置灰不可点击。"],
  ...
  },
  {
    ...
    title: "转写与生成",
    priority: "P0",
    goal: "作为文件转写与 AI 笔记生成前的方式选择和用量确认弹层。",
    entry: "文件列表点击生成、文件详情为空点击生成。",
    prototypeLinks: [["选择模板", "APP-REC-03"], ["音频语言", "APP-AI-02"], ["AI 模型", "APP-AI-03"], ["生成中", "APP-FILE-15"]],
    fields: [["generation_mode", "生成方式", "用户选择"], ["template_id", "生成模板", "自动模式系统选择/自定义模式用户必选"], ["template_name", "模板名称", "模板服务"], ["estimated_minutes", "预计消耗", "用量服务"], ["remaining_minutes", "剩余额度", "用量服务"], ["speaker_diarization_enabled", "自动标注说话人", "用户选择"], ["audio_language", "音频语言", "用户选择"], ["ai_model", "AI 模型", "用户选择"]],
    rules: ["文件列表和文件详情为空的生成入口必须先进入本页。", "默认选择自动生成，自动生成不要求用户手动选择模板。", "选择自定义生成后展开模板、说话人、语言和 AI 模型，其中模板为必选项。", "自定义生成未选择模板时，模板行仅显示查看全部，不显示自动匹配；直接点击立即生成时停留当前页并提示请选择模板。", "从模板列表选择并确认后返回本页，模板行显示具体模板名称。", "校验通过后点击立即生成进入文件详情生成中页面。", "额度不足时立即生成按钮置灰并禁止提交。", "弹层打开时底层文件详情整体进入同一蒙版状态，包括吸顶的大纲、智能总结、转写记录等导航。", "页面不重复展示录音场景，也不在按钮上方展示云端上传说明。"],
    states: ["自动生成（默认）", "自定义生成未选模板", "请选择模板提示", "模板已选择", "额度充足", "额度不足", "语言选择", "模型选择"],
    deps: ["用量服务：预估分钟、余额。", "会员服务：订阅等级与权益。", "支付服务：购买分钟数、升级订阅。", "AI 服务：模板列表、模型列表。", "云端文件：上传凭证。"],
    acceptance: ["首次进入默认勾选自动生成。", "选择自定义生成后模板为必选，空值显示查看全部且不出现自动匹配。", "未选择模板点击立即生成时显示请选择模板且不提交。", "选择模板进入无标签栏的分类模板列表，确认后模板行显示模板名称。", "底层大纲导航不得穿透蒙版或保持高亮。", "额度充足且配置校验通过时立即生成可提交并进入文件详情生成中。"]
  },
  {
    id: "APP-AI-02",
    group: "AI",
    title: "音频语言",
    priority: "P0",
    goal: "选择转写使用的音频语言或交由系统自动识别。",
    entry: "转写与生成页面点击音频语言。",
    fields: [["audio_language", "音频语言", "用户选择"]],
    rules: ["默认选择自动。", "弹层打开时底层文件详情整体进入蒙版状态，包括吸顶内容导航。", "选择语言后立即返回转写与生成页面并更新当前值。"],
    states: ["自动", "中文简体", "English (US)", "日本語", "其他热门语言"],
    deps: ["转写服务语言列表。"],
    acceptance: ["当前语言有清晰勾选态。", "底层大纲、智能总结、转写记录等导航不得穿透蒙版。", "点击任一语言后返回转写与生成页面。"]
  },
  {
    id: "APP-AI-03",
    group: "AI",
    title: "AI 模型",
    priority: "P0",
    goal: "选择用于生成 AI 笔记的模型或交由系统自动匹配。",
    entry: "转写与生成页面点击 AI 模型。",
    fields: [["ai_model", "AI 模型", "用户选择/系统策略"]],
    rules: ["默认选择自动。", "弹层打开时底层文件详情整体进入蒙版状态，包括吸顶内容导航。", "每个模型展示简短适用说明。", "选择模型后立即返回转写与生成页面并更新当前值。"],
    states: ["自动", "GPT-5", "GPT-5.5", "Gemini 2.5 Pro", "Gemini 3.5 Flash", "Claude Sonnet 5", "Claude Opus 4.8"],
    deps: ["AI 服务模型列表与可用状态。"],
    acceptance: ["当前模型有清晰勾选态。", "底层大纲、智能总结、转写记录等导航不得穿透蒙版。", "点击任一模型后返回转写与生成页面。"]
  },
  {
    id: "APP-ME-02",
    group: "我的",
  ...
  },
  {
    ...
    entry: "我的页点击账号设置。",
    fields: [["profile_avatar", "头像", "账号系统"], ["usage_stats", "使用统计", "本地数据库/用量服务"], ["recording_activity_heatmap", "录音卡实际录音使用热力图", "本地录音数据库"], ["preference_settings", "偏好设置", "本地设置"], ["account", "账号", "账号系统"]],
      rules: ["顶部展示头像、账号名、使用天数、录音总数和总使用时长。", "热力图只统计录音卡实际录音使用，不使用 App 打开次数；按连续周横向排列、每周固定 7 天，默认展示最近 53 周。", "小屏允许横向滚动查看更早日期；进入页面时自动定位到最右侧。热力图仅展示方格，不展示标题、日期范围、图例或当前日期选中态。", "热力图下方展示 10px-11px 灰色月份刻度，月份刻度与周列对齐，并随热力图一起横向滚动。", "每个热力图格提供日期和录音时长的 title 与 aria-label。", "底部列表包含偏好设置和账号入口。", "点击账号进入账号管理页。"],
      states: ["默认账号", "最近53周录音使用", "横向滚动", "头像编辑入口", "偏好设置入口", "账号入口"],
    deps: ["账号系统：用户头像与账号名。", "本地数据库：录音统计。"],
      acceptance: ["设置页样式参考截图，偏好设置和账号入口均可进入。", "热力图纵向固定 7 天、末列完整填满，且无当前日期选中态。", "热力图下方月份刻度可见，字号为 10px-11px 灰色，并在横向滚动时与热力图同步移动。", "热力图内容明确为录音卡实际录音使用。"],
    prototypeLinks: [["偏好设置", "APP-ME-26"], ["账号", "APP-ME-17"]]
  },
  {
    id: "APP-ME-26",
    group: "设置",
    title: "偏好设置",
    priority: "P1",
    goal: "集中管理转写、说话人、术语、通知、安全与 AI 改进偏好。",
    entry: "设置页点击偏好设置。",
    fields: [["transcription_language", "转写语言", "本地设置"], ["improve_readability", "自动提升转写可读性", "本地设置"], ["speaker_diarization", "自动标注说话人", "本地设置/声音样本"], ["custom_terms", "自定义术语", "本地设置"], ["notifications", "消息与通知", "系统通知权限/本地设置"], ["app_lock", "应用锁", "系统生物识别权限/本地设置"], ["ai_improvement", "帮助改进 AI", "用户授权"]],
    rules: ["每个下级页面均可返回偏好设置。", "自动提升转写可读性和帮助改进 AI 使用即时状态渲染。", "没有行业或常用术语时，启用自定义术语前展示说明。"],
    states: ["默认", "开关更新", "缺少行业与术语提示"],
    deps: ["本地设置、系统通知权限、系统生物识别权限、AI 服务授权。"],
    acceptance: ["七组偏好均有可操作入口或开关。"],
    prototypeLinks: [["转写语言", "APP-ME-27"], ["自动标注说话人", "APP-ME-28"], ["自定义术语", "APP-ME-34"], ["消息与通知", "APP-ME-37"], ["应用锁", "APP-ME-38"]]
  },
  {
    id: "APP-ME-27",
    group: "设置",
    ...
    goal: "选择录音转写的默认语言。",
    entry: "偏好设置点击转写语言。",
    fields: [["transcription_language", "默认转写语言", "用户选择"]],
    rules: ["选择后立即保存并返回偏好设置。", "支持自动检测和常用语言。"],
    states: ["当前语言", "已选择"],
    deps: ["转写服务语言列表。"],
    acceptance: ["当前选项有明确勾选状态。"]
  },
  {
    id: "APP-ME-28",
    group: "设置",
    title: "自动标注说话人",
    priority: "P1",
    goal: "管理说话人自动标注与我的声音样本。",
    entry: "偏好设置点击自动标注说话人。",
    fields: [["speaker_diarization", "自动标注说话人", "本地设置"], ["voice_samples", "我的声音样本", "本地声音样本"]],
    rules: ["开启后在转写结果中区分说话人。", "我的声音样本进入命名、权限、录制和完成链路。"],
    states: ["开启", "关闭", "声音样本入口"],
    deps: ["说话人分离服务、麦克风权限。"],
    acceptance: ["开关可操作，我的声音样本入口可进入。"],
    prototypeLinks: [["我的声音样本", "APP-ME-29"]]
  },
  {
    id: "APP-ME-29",
    group: "设置",
    title: "我的声音样本",
    priority: "P1",
    goal: "查看或创建用于识别本人的声音样本。",
    entry: "自动标注说话人页点击我的声音样本。",
    fields: [["speaker_name", "样本名称", "本地设置"], ["voice_sample_state", "声音样本状态", "本地声音样本"]],
    rules: ["未创建时进入命名页。", "已创建时可重新录制。"],
    states: ["未创建", "已创建"],
    deps: ["本地声音样本。"],
    acceptance: ["创建和重新录制均有入口。"],
    prototypeLinks: [["命名", "APP-ME-30"]]
  },
  {
    id: "APP-ME-30",
    group: "设置",
  ...
  },
  {
    ...
    title: "麦克风权限",
    priority: "P1",
    goal: "模拟录制声音样本前的系统麦克风授权。",
    entry: "命名声音样本后。",
    fields: [["microphone_permission", "麦克风权限", "系统权限"]],
    rules: ["允许后进入录制样本，暂不允许则返回我的声音样本。"],
    states: ["待授权", "允许", "暂不允许"],
    deps: ["系统麦克风权限。"],
    acceptance: ["权限弹窗有允许、拒绝和关闭入口。"],
    prototypeLinks: [["允许并录制", "APP-ME-32"]]
  },
  {
    id: "APP-ME-32",
    group: "设置",
  ...
  },
  {
    ...
    title: "声音样本完成",
    priority: "P1",
    goal: "确认声音样本已创建并返回样本列表。",
    entry: "完成录制声音样本后。",
    fields: [["voice_sample_state", "声音样本完成状态", "本地声音样本"]],
    rules: ["完成后样本列表显示已创建状态。"],
    states: ["完成"],
    deps: ["本地声音样本。"],
    acceptance: ["可返回我的声音样本。"]
  },
  {
    id: "APP-ME-34",
    group: "设置",
    title: "自定义术语",
    priority: "P1",
    goal: "按行业维护常用术语，提升转写准确性与可读性。",
    entry: "偏好设置点击自定义术语。",
    fields: [["custom_terms_enabled", "启用自定义术语", "本地设置"], ["industry", "行业", "用户选择"], ["terms", "常用术语列表", "用户维护"]],
    rules: ["支持行业选择、术语添加、编辑和删除。", "开启自定义术语前必须先选择行业或添加自定义术语。", "未完成配置时点击开启，展示底部弹窗提示并保持关闭。"],
    states: ["关闭", "开启", "未配置拦截", "无行业与术语", "有行业", "有术语"],
    deps: ["本地设置、转写术语词典。"],
    acceptance: ["自定义术语总览页只展示行业与常用术语入口，不重复展开术语列表。"],
    prototypeLinks: [["行业选择", "APP-ME-35"], ["常用术语", "APP-ME-36"]]
  },
  {
    id: "APP-ME-35",
    group: "设置",
    ...
    goal: "选择用于优化转写的行业词库。",
    entry: "自定义术语点击行业。",
    fields: [["industry", "行业", "用户选择"]],
    rules: ["可选择不设置行业。", "选择后保存并返回自定义术语。"],
    states: ["已选择", "无行业"],
    deps: ["行业词库配置。"],
    acceptance: ["当前行业有勾选状态。"]
  },
  {
    id: "APP-ME-36",
    group: "设置",
    title: "常用术语",
    priority: "P1",
    goal: "查看、添加、编辑和删除常用术语。",
    entry: "自定义术语点击常用术语。",
    fields: [["terms", "常用术语列表", "用户维护"], ["term_editor_sheet", "术语编辑面板", "页面弹层"]],
    rules: ["空列表展示空态和右上角添加入口。", "已有术语按首字符分组展示。", "添加和编辑在底部面板中完成，不把 APP-ME-36 本身做成输入页。"],
    states: ["空列表", "术语列表", "添加面板", "编辑面板", "删除术语"],
    deps: ["本地设置。"],
    acceptance: ["常用术语列表、添加、编辑和删除均可操作。"]
  },
  {
    id: "APP-ME-37",
    group: "设置",
    ...
    goal: "独立控制不同类型的应用通知。",
    entry: "偏好设置点击消息与通知。",
    fields: [["notifications_allowed", "允许通知", "通知设置"], ["progress_account_alerts", "进度与账号提醒", "通知设置"], ["device_alerts", "设备状态提醒", "通知设置"], ["product_updates", "产品更新与优惠", "通知设置"]],
    rules: ["允许通知作为总开关。", "各通知类型可独立开关，总开关关闭时类型项不可操作。"],
    states: ["允许通知", "关闭通知", "每类开启", "每类关闭"],
    deps: ["系统通知权限、本地设置。"],
    acceptance: ["每个通知类型均有独立开关。"]
  },
  {
    id: "APP-ME-38",
    group: "设置",
    title: "应用锁",
    priority: "P1",
    goal: "使用 Face ID 保护应用，并设置自动锁定时长。",
    entry: "偏好设置点击应用锁。",
    fields: [["app_lock_enabled", "应用锁", "本地设置"], ["face_id_permission", "Face ID 权限", "系统权限"], ["auto_lock_duration", "自动锁定时长", "本地设置"]],
    rules: ["首次开启先进入 Face ID 权限模拟。", "开启后在当前页直接选择自动锁定时长。"],
    states: ["关闭", "请求 Face ID", "开启", "选择自动锁定时长"],
    deps: ["系统生物识别权限、本地设置。"],
    acceptance: ["Face ID 权限与自动锁定时长均可操作。"],
    prototypeLinks: [["Face ID 权限", "APP-ME-40"]]
  },
  {
    id: "APP-ME-39",
    group: "设置",
    title: "自动锁定时长",
    priority: "P1",
    goal: "设置应用进入后台后的自动锁定时间。",
    entry: "应用锁开启后点击自动锁定。",
    fields: [["auto_lock_duration", "自动锁定时长", "用户选择"]],
    rules: ["选择后立即保存并返回应用锁。"],
    states: ["立即", "1分钟后", "5分钟后", "15分钟后"],
    deps: ["本地设置。"],
    acceptance: ["当前时长有明确勾选状态。"]
  },
  {
    id: "APP-ME-40",
    group: "设置",
    title: "Face ID 权限",
    priority: "P1",
    goal: "模拟首次开启应用锁时的 Face ID 系统授权。",
    entry: "应用锁关闭时点击开启。",
    fields: [["face_id_permission", "Face ID 权限", "系统权限"]],
    rules: ["允许后开启应用锁并返回应用锁页。", "暂不允许保持关闭。"],
    states: ["待授权", "允许", "暂不允许"],
    deps: ["系统 Face ID 权限。"],
    acceptance: ["权限弹窗有允许、拒绝和关闭入口。"]
  },
...
if (currentPageId === "APP-SYNC-02") currentPageId = "APP-SYNC-01";
...
let draftTemplateId = "";
let templateReturnPage = "APP-AI-01";
let generationNotice = "";
let recordMode = "auto";
let selectedRecordScene = "meeting";
let generationSpeakerDiarization = true;
let selectedAudioLanguage = "自动";
...
function createDefaultHomeSyncQueue() {
  return [
    { id: "sync-01", title: "Design workshop", size: 18, transferred: 18, status: "completed" },
    { id: "sync-02", title: "Weekly planning", size: 24, transferred: 24, status: "completed" },
    { id: "sync-03", title: "Customer discovery", size: 28, transferred: 28, status: "completed" },
    { id: "sync-04", title: "周一项目复盘", size: 32, transferred: 14.7, status: "syncing" },
    { id: "sync-05", title: "客户访谈", size: 20, transferred: 7, status: "pending" },
    { id: "sync-06", title: "Launch checklist", size: 18, transferred: 17.3, status: "pending" },
    { id: "sync-07", title: "Partner interview", size: 35, transferred: 2.4, status: "pending" },
    { id: "sync-08", title: "Research debrief", size: 42, transferred: 0, status: "pending" },
    { id: "sync-09", title: "Team retrospective", size: 27, transferred: 0, status: "pending" },
    { id: "sync-10", title: "Campaign review", size: 21, transferred: 0, status: "pending" },
    { id: "sync-11", title: "Feature planning", size: 16, transferred: 0, status: "pending" },
    { id: "sync-12", title: "Quarterly summary", size: 40, transferred: 0, status: "pending" }
  ];
}
let homeSyncQueue = createDefaultHomeSyncQueue();
let homeFailedFileState = "failed";
...
let homePriorityMenuTaskId = "";
let homePriorityMenuTimer = null;
let homePriorityNotice = "";
let homePriorityNoticeTimer = null;
let homePriorityLongPressTimer = null;
...
let customTerms = [];
let termEditorIndex = null;
let settingsNotice = "";
let pendingTermsEnableNotice = false;
let termSheetMode = "";
let termSheetError = "";
...
function clearFirmwareUpdateTimer() {
...
function scheduleFirmwareUpdateStep(callback, delay = 3000) {
  ...
  firmwareUpdateTimer = window.setTimeout(() => {
    firmwareUpdateTimer = null;
    if (currentPageId !== "APP-DEV-08") return;
    callback();
  }, delay);
}

function beginFirmwareTransfer() {
  firmwareUpdateStep = "transfer";
  firmwareJoinPromptOpen = false;
  render();
  scheduleFirmwareUpdateStep(() => {
    if (firmwareUpdateStep !== "transfer") return;
    firmwareUpdateStep = "install";
    render();
  });
}

function beginFirmwareUpdateFlow() {
  clearFirmwareUpdateTimer();
  firmwareUpdateStep = "download";
  firmwareJoinPromptOpen = false;
  scheduleFirmwareUpdateStep(() => {
    if (firmwareUpdateStep !== "download") return;
    firmwareUpdateStep = "connect";
    if (firmwareUpdateTransport === "wifi") {
      firmwareJoinPromptOpen = true;
      render();
      return;
    }
    render();
    scheduleFirmwareUpdateStep(() => {
      if (firmwareUpdateStep !== "connect") return;
      beginFirmwareTransfer();
    });
  });
}

function resumeFirmwareUpdateAtConnection() {
  clearFirmwareUpdateTimer();
  firmwareUpdateStep = "connect";
  firmwareJoinPromptOpen = firmwareUpdateTransport === "wifi";
  if (firmwareUpdateTransport === "bluetooth") {
    scheduleFirmwareUpdateStep(() => {
      if (firmwareUpdateStep !== "connect") return;
      beginFirmwareTransfer();
    });
...
function go(id) {
  ...
  if (currentPageId === "APP-DEV-08" && previousPageId !== "APP-DEV-08") {
    if (firmwareRetryAtConnection) {
      resumeFirmwareUpdateAtConnection();
    } else {
      beginFirmwareUpdateFlow();
    }
    firmwareRetryAtConnection = false;
  }
  if (currentPageId !== "APP-HOME-01") {
    homeDeviceMenuOpen = false;
    homePriorityMenuTaskId = "";
    clearHomePriorityMenuTimer();
    clearHomePriorityLongPressTimer();
  }
  if (!["APP-DEV-02", "APP-DEV-04", "APP-DEV-05"].includes(currentPageId)) {
    deviceSnTipOpen = false;
  }
  ...
  if (currentPageId === "APP-FILE-23" && previousPageId !== "APP-FILE-23") {
    recycleSelectionMode = false;
    selectedRecycleIds = new Set();
    recycleDeleteConfirmOpen = false;
    recycleNotice = "";
  }
  ...
  if (currentPageId !== "APP-REC-04") {
    recordingEditor = null;
    recordingCameraOpen = false;
    recordingPhotoViewerId = "";
    recordingCameraPrompt = false;
    recordingNotice = "";
  }
  if (!["APP-FILE-01", "APP-FILE-10", "APP-FILE-15"].includes(currentPageId)) {
    fileDetailMarkerEditor = null;
    fileDetailContentEditor = null;
    fileDetailPhotoViewerId = "";
    fileDetailNotice = "";
  }
  if (currentPageId === "APP-FILE-01" && previousPageId !== "APP-FILE-01") {
    fileDetailActiveTab = "summary";
    fileDetailSummaryTab = "summary";
  }
  if (currentPageId === "APP-FILE-10" && previousPageId !== "APP-FILE-10") {
    fileDetailActiveTab = "markers";
  }
  if (currentPageId === "APP-FILE-15" && previousPageId !== "APP-FILE-15") {
    fileDetailGeneratingInitialized = false;
  }
  render();
}
...
function fileCard(file) {
  const disabled = file.transferState === "syncing" || file.transferState === "pending";
  const statusNode = disabled
    ? file.transferState === "pending" && file.taskId
      ? `<div class="file-transfer-status-wrap ${homePriorityMenuTaskId === file.taskId ? "menu-open" : ""}">
          <button class="file-transfer-status file-transfer-status-action ${file.isPriority ? "prioritized" : ""}" data-action="open-home-priority-menu" data-task-id="${h(file.taskId)}" aria-haspopup="dialog" aria-expanded="${homePriorityMenuTaskId === file.taskId ? "true" : "false"}" aria-label="${h(file.name)}，${file.isPriority ? "已优先" : "等待传输"}，点击查看传输顺序操作">
            <span role="status" aria-live="polite">${h(file.isPriority ? "已优先" : file.statusText)}</span>
          </button>
          ${homePriorityMenuTaskId === file.taskId ? renderHomePriorityPopover(file.taskId) : ""}
        </div>`
      : `<div class="file-transfer-status"><span role="status" aria-live="polite">${h(file.statusText)}</span>${file.transferState === "syncing" ? '<em class="sync-state-spinner file-transfer-spinner" aria-hidden="true"></em>' : ""}</div>`
    : file.errorText
      ? `<button class="file-card-recovery" ${file.actionKey ? `data-action="${h(file.actionKey)}"` : `data-go="${file.go}"`} aria-label="${h(file.errorText)}，${h(file.action)}"><span>${h(file.errorText)}</span><b aria-hidden="true">·</b><strong>${h(file.action)}</strong></button>`
      : "";
  const content = `
    <div>
      <p class="file-name">${h(file.name)}</p>
      <div class="file-meta">${h(file.meta)}</div>
      <div class="file-tags">${chip(file.source, file.source === "硬件录音" ? "blue" : "purple")}${file.state ? chip(file.state, file.tone) : ""}</div>
    </div>
  `;
  return `
    <div class="file-card ${file.featured ? "featured" : ""} ${disabled ? `file-card-transfer-disabled ${file.transferState}` : ""} ${file.errorText ? "file-card-has-recovery" : ""}"${file.transferState === "syncing" ? ' aria-disabled="true"' : ""}>
      ${disabled || file.errorText ? `${content}${statusNode}` : `<button class="file-card-main" data-go="${h(file.go)}" aria-label="打开${h(file.name)}详情">${content}</button>`}
    </div>
  `;
}

function renderHomeTransferFileCards() {
  if (homeSyncStage !== "finishing") return "";
  const activeTask = homeSyncQueue.find((task) => task.status === "syncing");
  const pendingTasks = homeSyncQueue.filter((task) => task.status === "pending");
  const pendingMeta = [
    "今天 09:20 · 00:28:14",
    "今天 08:45 · 00:19:08",
    "昨天 18:10 · 00:35:12",
  ...
  return [
    activeTask ? fileCard({
      ...
      statusText: `正在传输… ${homeSyncTaskPercent(activeTask)}%`
    }) : "",
    ...pendingTasks.slice(0, 3).map((task, index) => fileCard({
      name: task.title,
      meta: pendingMeta[index] || "待同步录音",
      source: "硬件录音",
      transferState: "pending",
      statusText: "等待传输",
      taskId: task.id,
      isPriority: Boolean(task.priority)
    }))
  ].join("");
}

function clearHomePriorityMenuTimer() {
  if (!homePriorityMenuTimer) return;
  window.clearTimeout(homePriorityMenuTimer);
  homePriorityMenuTimer = null;
}

function clearHomePriorityLongPressTimer() {
  if (!homePriorityLongPressTimer) return;
  window.clearTimeout(homePriorityLongPressTimer);
  homePriorityLongPressTimer = null;
}

function openHomePriorityMenu(taskId, focusAction = false) {
  const task = homeSyncQueue.find((item) => item.id === taskId && item.status === "pending");
  if (!task || currentPageId !== "APP-HOME-01") return;
  clearHomePriorityMenuTimer();
  homePriorityMenuTaskId = taskId;
  render();
  if (focusAction) {
    requestAnimationFrame(() => {
      const menuAction = document.querySelector(`.file-priority-popover [data-action="confirm-home-priority"]`);
      const trigger = document.querySelector(`[data-action="open-home-priority-menu"][data-task-id="${taskId}"]`);
      (menuAction || trigger)?.focus();
    });
  }
  homePriorityMenuTimer = window.setTimeout(() => {
    homePriorityMenuTimer = null;
    if (homePriorityMenuTaskId !== taskId) return;
    const menu = document.querySelector(".file-priority-popover");
...
function closeHomePriorityMenu() {
  clearHomePriorityMenuTimer();
  homePriorityMenuTaskId = "";
}

function renderHomePriorityPopover(taskId) {
  const task = homeSyncQueue.find((item) => item.id === taskId && item.status === "pending");
  if (!task) return "";
  const pendingTasks = homeSyncQueue.filter((item) => item.status === "pending");
  const pendingIndex = pendingTasks.findIndex((item) => item.id === taskId);
  const isNext = pendingIndex === 0;
  const filesAhead = Math.max(1, pendingIndex + (homeSyncQueue.some((item) => item.status === "syncing") ? 1 : 0));
  const estimatedMinutes = Math.max(1, filesAhead * (homeSyncTransport === "hotspot" ? 1 : 2));
  return `
    <section class="file-priority-popover" role="dialog" aria-label="调整传输顺序">
      <strong>${isNext ? "当前已是下一项" : `前面还有 ${filesAhead} 个文件`}</strong>
      <span>${isNext ? "当前文件完成后将自动传输" : `预计约 ${estimatedMinutes} 分钟后开始`}</span>
      ${isNext ? "" : `<button data-action="confirm-home-priority" data-task-id="${h(taskId)}">设为优先同步</button>`}
    </section>
  `;
}

function setHomePriorityTask(taskId) {
  const taskIndex = homeSyncQueue.findIndex((item) => item.id === taskId && item.status === "pending");
  if (taskIndex < 0) return false;
  const pendingTasks = homeSyncQueue.filter((item) => item.status === "pending");
  if (pendingTasks[0]?.id === taskId) return false;
  const [task] = homeSyncQueue.splice(taskIndex, 1);
  homeSyncQueue.forEach((item) => {
    item.priority = false;
  });
  task.priority = true;
  const activeIndex = homeSyncQueue.findIndex((item) => item.status === "syncing");
  const insertIndex = activeIndex >= 0 ? activeIndex + 1 : homeSyncQueue.findIndex((item) => item.status === "pending");
  homeSyncQueue.splice(insertIndex >= 0 ? insertIndex : homeSyncQueue.length, 0, task);
  closeHomePriorityMenu();
  homePriorityNotice = "已设为优先，将在当前文件后传输";
...
function homeSyncStats() {
  const total = homeSyncQueue.length;
  const completed = homeSyncQueue.filter((task) => task.status === "completed").length;
  const totalBytes = homeSyncQueue.reduce((sum, task) => sum + task.size, 0);
...
function homeSyncTaskPercent(task) {
  return task.size ? Math.round((Math.min(task.transferred, task.size) / task.size) * 100) : 0;
}

function shouldShowHomeSyncStatus() {
  if (homeSyncStage !== "finishing" || !homeSyncQueue.length) return false;
  const stats = homeSyncStats();
  const unfinishedTaskCount = homeSyncQueue.filter((task) => task.status !== "completed").length;
  return !(unfinishedTaskCount < 2 && stats.remainingMinutes < 3);
}
  ...
  return `
    <section class="sync-mini finishing ${homeSyncTransport === "hotspot" ? "hotspot-active" : ""}" aria-label="当前同步状态">
      <div class="sync-mini-main" aria-label="本次同步进度">
        ...
        <i class="sync-transport-badge ${transport.tone}">${h(transport.label)}</i>
        <b>${stats.percent}%</b>
...
function completeCurrentHomeSyncTask() {
  ...
  activeTask.transferred = activeTask.size;
  const nextTask = homeSyncQueue.find((task) => task.status === "pending");
  if (nextTask) {
    nextTask.status = "syncing";
    nextTask.priority = false;
...
function startSingleHomeSyncBatch() {
  closeHomePriorityMenu();
  homeDeviceAdded = true;
  homeSyncBatchSerial += 1;
  homeSyncQueue = [{ id: `sync-new-${homeSyncBatchSerial}`, title: "New field interview", size: 22, transferred: 7.6, status: "syncing" }];
  homeSyncStage = "finishing";
}

function resetMultiHomeSyncBatch() {
  closeHomePriorityMenu();
  homeDeviceAdded = true;
  homeSyncBatchSerial += 1;
  homeSyncQueue = createDefaultHomeSyncQueue();
  homeFailedFileState = "failed";
  homeSyncStage = "finishing";
}

function showNoHomeSyncTasks() {
  closeHomePriorityMenu();
  homeDeviceAdded = true;
  homeSyncQueue = [];
  homeFailedFileState = "failed";
...
function showNoHomeDevice() {
  homeDeviceAdded = false;
  homeDeviceMenuOpen = false;
  closeHomePriorityMenu();
}
...
function snHelpButton() {
  return `<button class="sn-help-btn" data-action="open-sn-tip" aria-label="查看 SN 所在位置">?</button>`;
}
...
function renderDeviceBind() {
  return screen(`
    <section class="found-head">
      ...
      <p><span>SN: 8810B30293069513</span>${snHelpButton()}</p>
    </section>
...
function renderHomeContent({ showSync = true } = {}) {
  const savedRecordingCard = homeSyncStage === "saved" || homeSyncStage === "idle"
    ? fileCard({ featured: true, name: "周一项目复盘", meta: "刚刚 · 00:43:52", source: "硬件录音", state: "未转写", tone: "amber", go: "APP-FILE-10" })
    : "";
  const activeDevice = boundHomeDevices.find((device) => device.id === activeHomeDeviceId) || boundHomeDevices[0];
  const canShowDeviceSync = homeDeviceAdded && showSync;
  return `
    ${homeDeviceAdded ? `<section class="home-device-switcher ${homeDeviceMenuOpen ? "open" : ""}" aria-label="设备状态">
      ...
      ` : ""}
    </section>` : `<section class="home-device-switcher home-device-empty" aria-label="未添加录音卡">
      <button class="home-device-empty-action" data-go="APP-DEV-01" aria-label="添加设备，搜索并绑定新的 AI Recorder">
        <span class="home-device-empty-visual" aria-hidden="true"><span class="home-device-glyph"><i></i></span></span>
        <span class="home-device-empty-copy"><b>添加设备</b><small>搜索并绑定新的 AI Recorder</small></span>
      </button>
    </section>`}
    ${canShowDeviceSync ? renderHomeSyncModule() : ""}
    <section class="card file-list-panel" id="homeFileList">
      <div class="card-title sticky-list-title">
        <button class="list-title-filter" data-go="APP-FILE-03" aria-label="全部文件筛选和排序"><strong>全部文件</strong><span></span></button>
        <button class="import-btn" data-go="APP-FILE-13">+导入音频</button>
      </div>
      ${homeDeviceAdded ? renderHomeTransferFileCards() : ""}
      ${savedRecordingCard}
      ${homeFailedFileState === "failed"
        ? fileCard({ name: "产品讨论", meta: "昨天 16:40 · 00:58:20", source: "硬件录音", errorText: "同步失败", action: "重试", actionKey: "retry-home-failed-file" })
        : fileCard({ name: "产品讨论", meta: "昨天 16:40 · 00:58:20", source: "硬件录音", transferState: "syncing", statusText: "正在重试…" })}
      ${fileCard({ name: "Board meeting", meta: "今天 14:20 · 01:12:17 · 3 个标记", source: "硬件录音", state: "未转写", tone: "amber", go: "APP-FILE-10" })}
      ${fileCard({ name: "Interview import", meta: "昨天 19:10 · 00:34:02", source: "导入文件", state: "AI 已生成", tone: "green", go: "APP-FILE-01" })}
      ${fileCard({ name: "Product review", meta: "07-04 · 00:26:44 · 2 个标记", source: "导入文件", state: "未转写", tone: "amber", go: "APP-FILE-10" })}
...
function renderHomePriorityToast() {
  if (!homePriorityNotice) return "";
  return `
    <div class="home-priority-toast" role="status" aria-live="polite">
      <span>${h(homePriorityNotice)}</span>
    </div>
  `;
}

function renderHome() {
  return `${screen(renderHomeContent(), { title: "文件", action: "search", active: "files" })}${homePriorityMenuTaskId ? '<button class="home-priority-dismiss" data-action="close-home-priority-menu" aria-label="关闭传输顺序操作"></button>' : ""}${homeHotspotSuccessNotice ? renderHotspotSuccessToast() : ""}${renderHomePriorityToast()}`;
}
  ...
  return `
    ...
    <section class="filter-sheet">
      ...
      <div class="filter-options">
        ...
        <button class="filter-option" data-go="APP-FILE-23"><span class="file-icon trash"></span><b>回收站 <em>(${recycleBinItems.length})</em></b></button>
      </div>
      <div class="sheet-line"></div>
...
function renderRecycleBinItem(item) {
  const selected = selectedRecycleIds.has(item.id);
  return `
    <button class="recycle-file-row ${selected ? "selected" : ""}" data-action="toggle-recycle-item" data-recycle-id="${h(item.id)}" aria-pressed="${selected ? "true" : "false"}">
      <span class="recycle-file-copy">
        <strong>${h(item.name)}</strong>
        <small>${h(item.meta)}</small>
        <span>${h(item.source)} · 移入回收站 ${h(item.deletedAt)}</span>
      </span>
      ${recycleSelectionMode ? `<input type="checkbox" tabindex="-1" aria-hidden="true" ${selected ? "checked" : ""}>` : ""}
    </button>
  `;
}

function renderRecycleDeleteConfirm() {
  if (!recycleDeleteConfirmOpen) return "";
  const count = selectedRecycleIds.size;
  return `
    <div class="scrim recycle-confirm-scrim" data-action="cancel-recycle-delete"></div>
    <section class="recycle-delete-confirm" role="dialog" aria-modal="true" aria-label="永久删除确认">
      <h2>永久删除所选文件？</h2>
      <p>将永久删除已选择的 ${count} 个音频文件及其转写和 AI 笔记，此操作无法撤销。</p>
      <button class="danger-fill" data-action="confirm-recycle-delete">永久删除</button>
      <button class="outline" data-action="cancel-recycle-delete">取消</button>
    </section>
  `;
}
...
function renderRecycleBin() {
  ...
  return `
    <div class="statusbar"><span>9:41</span><span>5G</span><span>82%</span></div>
    <header class="recycle-page-header ${recycleSelectionMode ? "selecting" : ""}">
      ${recycleSelectionMode
        ? `<span class="recycle-header-spacer"></span><h1>已选择 ${count} 项</h1><button data-action="finish-recycle-selection">完成</button>`
        : `<button class="recycle-back" data-go="APP-HOME-01" aria-label="返回文件首页">‹</button><h1>回收站</h1>${hasItems ? `<button data-action="start-recycle-selection">选择</button>` : `<span class="recycle-header-spacer"></span>`}`}
    </header>
    <main class="recycle-page ${recycleSelectionMode ? "selecting" : ""}">
      ${hasItems ? `
        <p class="recycle-retention-note">文件将在移入回收站 30 天后自动删除</p>
        <section class="recycle-file-list" aria-label="回收站文件">
          ${recycleBinItems.map(renderRecycleBinItem).join("")}
        </section>
      ` : `
        <section class="recycle-empty-state" aria-label="回收站为空">
          <img src="./assets/icons/trash3.svg" alt="">
          <h2>暂无音频记录</h2>
          <p>移入回收站的音频会在这里保留 30 天</p>
        </section>
      `}
    </main>
    ${recycleSelectionMode ? (count > 0 ? `
      <nav class="recycle-action-bar" aria-label="回收站批量操作">
        <button data-action="restore-recycle-items">恢复</button>
        <button class="danger" data-action="request-recycle-delete">删除</button>
      </nav>
    ` : "") : bottom("files")}
    ${recycleNotice ? `<div class="toast-message recycle-toast" role="status">${h(recycleNotice)}</div>` : ""}
    ${renderRecycleDeleteConfirm()}
  `;
}
  ...
  return `
    ...
    <div class="search-page-top">
      ...
      <label class="search-input">
        ...
        <input id="searchInput" type="search" value="${h(searchDraft)}" placeholder="搜索或提问" autocomplete="off" spellcheck="false">
      </label>
    </div>
    <main class="screen-content search-screen compact">
      ...
      <button class="recent-chip" data-action="open-search-results" data-query="好">好</button>
      <h3 class="recent-days">最近 30 天</h3>
      <button class="search-result" data-action="open-search-results" aria-label="查看相关搜索结果">
...
function renderSearchResults() {
  const query = activeSearchQuery || "说";
  const highlight = (text) => String(text)
    .split(query)
    .map((part) => h(part))
    .join(`<mark>${h(query)}</mark>`);
  const results = [
    {
      title: "07-04 会议记录工具范式缺陷：音频与视觉信息未统一导致上下文断裂",
      excerpt: `做关联起来的，就比如${query}，假如现在我们真的是在开一个会，上面还有投影内容需要一起记录……`,
      type: "转写",
      time: "7月4日 15:18"
    },
    {
      title: "2026-06-29 16:31:40",
      excerpt: `这段录音围绕${query}展开，讨论了录音内容如何通过接口转换并保留完整上下文。`,
      type: "转写",
      time: "6月29日 16:31"
    },
    {
      title: `产品周会：${query}相关事项梳理`,
      excerpt: `AI 笔记已整理${query}相关结论、待办事项和后续需要验证的产品细节。`,
      type: "AI 笔记",
      time: "6月27日 10:06"
    }
  ];
  return `
    ...
    <div class="search-page-top result">
      ...
      <div class="search-input active"><span></span><b>${h(query)}</b></div>
    </div>
      ...
      <section class="search-result-list">
        ${results.map((result) => `
          <article class="search-result-item" data-go="APP-FILE-01">
            <h4>${highlight(result.title)}</h4>
            <p>${highlight(result.excerpt)}</p>
            <div><span>${h(result.type)}</span><time>${h(result.time)}</time></div>
          </article>
        `).join("")}
      </section>
    </main>
...
const templateCatalog = [
  {
    category: "通用",
    items: [
      { id: "smart-summary", title: "智能摘要", desc: "多场景自适应摘要，快速提炼要点" },
      { id: "reasoning-summary", title: "推理总结", desc: "自动生成结构化总结，快速把握要点" }
    ]
  },
  {
    category: "会议",
    items: [
      { id: "meeting-minutes", title: "会议纪要", desc: "整理会议要点与结论，生成行动项" },
      { id: "discussion-summary", title: "讨论总结", desc: "提炼讨论重点与分歧，明确结论与行动项" }
    ]
  },
  {
    category: "演讲",
    items: [
      { id: "class-notes", title: "课堂笔记", desc: "结构化记录课堂要点与作业，便于复习" },
      { id: "course-summary", title: "课程总结", desc: "提炼课程要点与建议，一页看懂讲座" }
    ]
  },
  {
    category: "通话",
    items: [
      { id: "phone-discussion", title: "电话讨论", desc: "记录电话讨论，清晰产出结论与待办" },
      { id: "phone-consulting", title: "电话咨询", desc: "整理咨询答复与后续行动" }
    ]
  },
  {
    category: "面试/采访",
    items: [
      { id: "research-interview", title: "研究访谈", desc: "整理访谈要点，快速回顾关键信息" },
      { id: "interview-analysis", title: "面试分析", desc: "整理面试纪要，输出要点与行动项" }
    ]
  }
];

function templateById(id) {
  return templateCatalog.flatMap((group) => group.items).find((item) => item.id === id) || null;
}

function selectedTemplateLabel() {
  return templateById(selectedTemplateId)?.title || "";
}

const recordSceneOptions = [
  { id: "meeting", title: "会议", desc: "优化说话人分离与重点提炼" },
  { id: "interview", title: "访谈", desc: "适合一对一或多人访谈，强化问答结构识别" },
  { id: "classroom", title: "课堂", desc: "适合教学场景，整理知识点与复习清单" },
  { id: "phone", title: "电话", desc: "适合通话场景，提升通话内容的转写准确率" }
];

function renderRecordSheet() {
  return `
    ${renderHome()}
    <div class="scrim record-prepare-scrim" data-go="APP-HOME-01"></div>
    <section class="bottom-sheet record-prepare-sheet mode-${recordMode}" role="dialog" aria-modal="true" aria-labelledby="recordPrepareTitle">
      <div class="handle"></div>
      <header class="record-prepare-header">
        <h2 id="recordPrepareTitle">准备录音</h2>
        <p>AI Recorder A1 · 已连接</p>
      </header>
      <div class="record-mode-group" role="radiogroup" aria-label="录音场景识别方式">
        <button class="record-mode-option ${recordMode === "auto" ? "selected" : ""}" data-action="select-record-mode" data-value="auto" role="radio" aria-checked="${recordMode === "auto" ? "true" : "false"}">
          <span class="option-dot">${recordMode === "auto" ? "✓" : ""}</span>
          <span><strong>自动识别场景</strong><small>录音完成后自动匹配转写与总结</small></span>
        </button>
        <button class="record-mode-option ${recordMode === "manual" ? "selected" : ""}" data-action="select-record-mode" data-value="manual" role="radio" aria-checked="${recordMode === "manual" ? "true" : "false"}" aria-expanded="${recordMode === "manual" ? "true" : "false"}" aria-controls="recordSceneOptions">
          <span class="option-dot">${recordMode === "manual" ? "✓" : ""}</span>
          <span><strong>选择录音场景</strong><small>可指定会议、访谈、课堂或电话</small></span>
        </button>
        ${recordMode === "manual" ? `
          <div class="record-scene-options" id="recordSceneOptions" role="radiogroup" aria-label="录音场景">
            ${recordSceneOptions.map((scene) => `
              <button class="record-scene-option ${selectedRecordScene === scene.id ? "selected" : ""}" data-action="select-record-scene" data-value="${h(scene.id)}" role="radio" aria-checked="${selectedRecordScene === scene.id ? "true" : "false"}">
                <span class="option-dot">${selectedRecordScene === scene.id ? "✓" : ""}</span>
                <span><strong>${h(scene.title)}</strong><small>${h(scene.desc)}</small></span>
              </button>
            `).join("")}
          </div>
        ` : ""}
      </div>
      <footer class="record-prepare-footer">
        <button class="primary-btn record-start-btn" data-go="APP-REC-04">开始录音</button>
      </footer>
    </section>
  `;
}
...
function renderTemplateCard(item) {
  const selected = draftTemplateId === item.id;
  return `
    <button class="template-card ${selected ? "selected" : ""}" data-action="select-template" data-value="${h(item.id)}">
      ${selected ? '<span class="template-selected-mark">✓</span>' : ""}
      <strong>${h(item.title)}</strong>
      <p>${h(item.desc)}</p>
      <small>AI Recorder</small>
    </button>
  `;
}
...
function renderScene() {
  return `
    <div class="statusbar"><span>9:41</span><span>5G</span><span>82%</span></div>
    <header class="app-top template-library-top">
      <button class="icon-btn" data-action="close-template-library" aria-label="返回">‹</button>
      <h2>选择模板</h2>
      <button class="icon-btn search-top-btn" aria-label="搜索模板"></button>
    </header>
    <main class="template-library-page">
      ${templateCatalog.map((group) => `
        <section class="template-category">
          <header><h3>${h(group.category)}</h3><button>查看更多 ›</button></header>
          <div class="template-grid">${group.items.map(renderTemplateCard).join("")}</div>
        </section>
      `).join("")}
    </main>
    <footer class="template-library-footer">
      <button class="primary-btn ${draftTemplateId ? "" : "disabled"}" ${draftTemplateId ? 'data-action="apply-template"' : "disabled"}>使用此模板</button>
    </footer>
  `;
}
...
function nextRecordingMarkTime() {
  ...
  const totalSeconds = (30 * 60) + 51 + (offset * 13);
  const minutes = Math.floor(totalSeconds / 60);
  const seconds = totalSeconds % 60;
  return `00:${String(minutes).padStart(2, "0")}:${String(seconds).padStart(2, "0")}`;
}

function nextRecordingMarkId() {
  recordingMarkSerial += 1;
  return `recording-mark-${recordingMarkSerial}`;
}

function showRecordingNotice(message) {
  recordingNotice = message;
  if (recordingNoticeTimer) window.clearTimeout(recordingNoticeTimer);
  recordingNoticeTimer = window.setTimeout(() => {
    recordingNoticeTimer = null;
    recordingNotice = "";
    if (currentPageId === "APP-REC-04") render();
  }, 1600);
}

function openRecordingEditor(mode, markId = "") {
  const mark = recordingMarks.find((item) => item.id === markId);
  const draft = mode === "new-input"
    ? ""
    : mode === "photo-caption"
      ? mark?.caption || ""
      : mark?.text || "";
  recordingEditor = {
    ...
    markId,
    draft,
    time: mark?.time || nextRecordingMarkTime()
  };
  recordingCameraPrompt = false;
}
...
function completeRecordingEditor() {
  if (!recordingEditor) return false;
  const draft = String(recordingEditor.draft || "").trim();
  if (recordingEditor.mode !== "photo-caption" && !draft) {
    showRecordingNotice("请输入标记内容");
    return false;
  }
  if (recordingEditor.mode === "new-input") {
    recordingMarks = [
      ...recordingMarks,
      { id: nextRecordingMarkId(), time: recordingEditor.time, type: "input", text: draft, source: "APP" }
    ];
    showRecordingNotice("已添加输入标记");
  } else if (recordingEditor.mode === "photo-caption") {
    recordingMarks = recordingMarks.map((mark) => mark.id === recordingEditor.markId
      ? { ...mark, caption: draft, captionSource: draft ? "user" : "" }
      : mark);
    showRecordingNotice(draft ? "照片说明已保存" : "照片说明已清空");
  } else {
    recordingMarks = recordingMarks.map((mark) => mark.id === recordingEditor.markId
      ? { ...mark, text: draft }
      : mark);
    showRecordingNotice("标记内容已保存");
  }
...
function addRecordingMark(type) {
  if (type === "input") {
    openRecordingEditor("new-input");
    return;
  }
  if (type === "photo") {
    recordingCameraOpen = true;
    recordingCameraPrompt = false;
    return;
  }
  recordingMarks = [
    ...recordingMarks,
    { id: nextRecordingMarkId(), time: nextRecordingMarkTime(), type: "mark", text: "重点标记", source: "APP" }
  ];
  showRecordingNotice("已添加时间标记");
}

function captureRecordingPhoto() {
  recordingMarks = [
    ...recordingMarks,
    {
      id: nextRecordingMarkId(),
      time: nextRecordingMarkTime(),
      type: "photo",
      text: "",
      caption: "",
      captionSource: "",
      source: "APP",
      photo: recordingPhotoAsset
    }
  ];
  recordingCameraOpen = false;
  recordingCameraPrompt = true;
  showRecordingNotice("照片标记已插入");
}

function simulateRecordingPhotoAiCaption() {
  const target = [...recordingMarks].reverse().find((mark) => mark.type === "photo" && !mark.caption && mark.captionSource !== "user");
  if (!target) {
    showRecordingNotice("没有可生成说明的照片");
    return;
  }
  recordingMarks = recordingMarks.map((mark) => mark.id === target.id
    ? { ...mark, caption: "会议笔记记录了项目进展、待办事项和下一步计划。", captionSource: "ai" }
    : mark);
  showRecordingNotice("AI 已生成照片说明");
}

function recordingKeyboardMock() {
  const suggestions = ["客户", "希望", "首版", "支持", "离线转写"];
  const rows = [
    ["Q", "W", "E", "R", "T", "Y", "U", "I", "O", "P"],
    ["A", "S", "D", "F", "G", "H", "J", "K", "L"],
    ["Z", "X", "C", "V", "B", "N", "M"]
  ];
  return `<section class="recording-keyboard" aria-label="中文输入键盘">
    <div class="recording-keyboard-suggestions">
      ${suggestions.map((word) => `<button type="button" data-action="recording-keyboard-key" data-key="${h(word)}">${h(word)}</button>`).join("")}
    </div>
    ${rows.map((row) => `<div class="recording-keyboard-row">${row.map((key) => `<button type="button" data-action="recording-keyboard-key" data-key="${key.toLowerCase()}">${key}</button>`).join("")}</div>`).join("")}
    <div class="recording-keyboard-row recording-keyboard-bottom">
      <button type="button" data-action="recording-keyboard-key" data-key="switch">123</button>
      <button type="button" class="space" data-action="recording-keyboard-key" data-key="space">空格</button>
      <button type="button" data-action="recording-keyboard-key" data-key="backspace">删除</button>
      <button type="button" data-action="complete-recording-editor">完成</button>
    </div>
  </section>`;
}

function renderRecordingEditorDock() {
  if (!recordingEditor) return "";
  const targetMark = recordingMarks.find((mark) => mark.id === recordingEditor.markId);
  const isCaption = recordingEditor.mode === "photo-caption";
  const canDeleteText = recordingEditor.mode === "edit-text" && targetMark?.type === "mark" && Boolean(targetMark.text);
  const placeholder = isCaption ? "添加照片说明（可选）" : "输入标记内容";
  const canComplete = isCaption || Boolean(String(recordingEditor.draft || "").trim());
  return `<section class="recording-editor-dock" aria-label="标记文字编辑">
    <div class="recording-editor-composer">
      ${canDeleteText ? `<button type="button" class="recording-delete-copy" data-action="delete-recording-mark-text" aria-label="删除标记文字"><img src="./assets/icons/trash3.svg" alt=""></button>` : ""}
      <input id="recordingEditorInput" value="${h(recordingEditor.draft)}" placeholder="${h(placeholder)}" autocomplete="off">
      <button type="button" class="recording-editor-cancel" data-action="cancel-recording-editor">取消</button>
      <button type="button" class="recording-editor-complete" data-action="complete-recording-editor" ${canComplete ? "" : "disabled"}>完成</button>
    </div>
    ${recordingKeyboardMock()}
  </section>`;
}

function renderRecordingMarks() {
  return recordingMarks.map((mark) => {
    const isExpanded = recordingExpandedMarkIds.has(mark.id);
    const isLong = String(mark.text || "").length > 34;
    const text = mark.text || "添加标记说明";
    return `<article class="timeline-item recording-marker ${mark.type === "photo" ? "photo-marker" : ""} ${mark.type === "input" ? "input-marker" : ""}" data-marker-id="${h(mark.id)}">
      <div class="recording-marker-meta"><strong>${h(mark.time)}</strong><span>${h(mark.source || "APP")}</span></div>
      ${mark.type === "photo" ? `
        <div class="recording-photo-wrap">
          <button type="button" class="recording-photo-preview" data-action="open-recording-photo" data-marker-id="${h(mark.id)}" aria-label="打开照片">
            <img src="${h(mark.photo || recordingPhotoAsset)}" alt="会议笔记照片">
          </button>
          <button type="button" class="recording-photo-delete" data-action="delete-recording-photo" data-marker-id="${h(mark.id)}" aria-label="删除照片标记"><img src="./assets/icons/trash3.svg" alt=""></button>
        </div>
        <button type="button" class="recording-photo-caption ${mark.caption ? "filled" : "empty"}" data-action="edit-recording-photo-caption" data-marker-id="${h(mark.id)}">
          <span>${h(mark.caption || "添加照片说明（可选）")}</span>
          <small>${mark.captionSource === "ai" ? "AI 生成 · 点击可编辑" : "转写后可由 AI 识别生成，点击可编辑"}</small>
        </button>
      ` : `
        <button type="button" class="recording-marker-copy ${!mark.text ? "empty" : ""} ${isLong && !isExpanded ? "collapsed" : ""}" data-action="edit-recording-mark" data-marker-id="${h(mark.id)}">${h(text)}</button>
        ${isLong ? `<button type="button" class="recording-expand-copy" data-action="toggle-recording-mark-expanded" data-marker-id="${h(mark.id)}">${isExpanded ? "收起" : "展开"}</button>` : ""}
      `}
    </article>`;
  }).join("");
}

function renderRecordingCamera() {
  if (!recordingCameraOpen) return "";
  return `<section class="recording-camera" aria-label="系统相机模拟">
    <header><button type="button" data-action="close-recording-camera">取消</button><strong>相机</strong><span></span></header>
    <div class="recording-camera-preview"><img src="${recordingPhotoAsset}" alt="相机取景：会议笔记"></div>
    <footer><button type="button" class="recording-camera-shutter" data-action="capture-recording-photo" aria-label="拍照"></button></footer>
  </section>`;
}

function renderRecordingPhotoViewer() {
  const mark = recordingMarks.find((item) => item.id === recordingPhotoViewerId && item.type === "photo");
  if (!mark) return "";
  return `<section class="recording-photo-viewer" aria-label="照片预览">
    <header><button type="button" data-action="close-recording-photo">关闭</button><strong>${h(mark.time)} · ${h(mark.source)}</strong><span></span></header>
    <img src="${h(mark.photo || recordingPhotoAsset)}" alt="会议笔记照片大图">
    <p>${h(mark.caption || "尚未添加照片说明")}</p>
  </section>`;
}
...
function renderRecording() {
  return `
    <section class="recording-stage">
      ...
      <main class="recording-panel ${recordingEditor ? "is-editing" : ""}">
        ...
        <div class="recording-top">
          ...
          <button data-action="finish-recording-sync">结束</button>
        </div>
        <div class="recording-device-summary">
          <i aria-hidden="true"></i>
          <strong>AI Recorder A1</strong>
          <span>已连接 · 76% · 11.8G</span>
        </div>
        ...
        <section class="recording-mark-card">
          <h3>标记记录</h3>
          ${recordingCameraPrompt ? `<p class="recording-camera-toast">照片已插入，可点击查看或补充说明</p>` : ""}
          <div class="timeline" role="list">
            ${renderRecordingMarks()}
          </div>
        </section>
        ${recordingEditor ? "" : `<div class="recording-actions">
      ...
      ${recordingNotice ? `<p class="recording-notice" role="status">${h(recordingNotice)}</p>` : ""}
      ${renderRecordingEditorDock()}
      ${renderRecordingCamera()}
      ${renderRecordingPhotoViewer()}
    </section>
  `;
}
  ...
  return `
    <section class="ask-answer">
      ...
      <section class="ask-followups">
        <h3>继续提问</h3>
        ${askPlaudFollowups.map((question) => `<button data-action="ask-suggestion" data-question="${h(question)}">${h(question)}</button>`).join("")}
      </section>
    </section>
  `;
}
...
function renderAskPlaudSheet() {
  if (askPlaudMode === "closed") return "";
  const body = askPlaudMode === "thinking"
...
function fileDetailTimeToSeconds(time) {
  return String(time || "00:00:00").split(":").reduce((total, part) => total * 60 + Number(part || 0), 0);
}

function fileDetailSecondsToTime(totalSeconds) {
...
function showFileDetailNotice(message) {
  fileDetailNotice = message;
  if (fileDetailNoticeTimer) window.clearTimeout(fileDetailNoticeTimer);
  fileDetailNoticeTimer = window.setTimeout(() => {
    fileDetailNoticeTimer = null;
    fileDetailNotice = "";
    if (["APP-FILE-01", "APP-FILE-10", "APP-FILE-15"].includes(currentPageId)) render();
  }, 1800);
}

function renderFileDetailPlayer(totalTime = "00:43:52") {
  const duration = Math.max(1, fileDetailTimeToSeconds(totalTime));
  const progress = Math.min(100, Math.max(0, fileDetailAudioSeconds / duration * 100));
  return `
    <section class="audio-player file-marker-player">
      <div class="audio-progress-row">
        <span>${h(fileDetailSecondsToTime(fileDetailAudioSeconds))}</span>
        <button class="audio-track" data-action="file-detail-seek-track" data-duration="${duration}" aria-label="调整播放进度"><b style="left: ${progress}%"></b></button>
        <span>${h(totalTime)}</span>
      </div>
      <div class="player-controls">
        <button class="player-icon sliders" data-go="APP-FILE-04" aria-label="音频设置"></button>
        <button class="player-icon rewind" data-action="file-detail-skip" data-seconds="-15" aria-label="快退 15 秒">15</button>
        <button class="play-icon ${fileDetailAudioPlaying ? "playing" : ""}" data-action="file-detail-toggle-play" aria-label="${fileDetailAudioPlaying ? "暂停" : "播放"}"></button>
        <button class="player-icon forward" data-action="file-detail-skip" data-seconds="15" aria-label="快进 15 秒">15</button>
...
function renderFileDetailMarkerItem(mark, markerState) {
  const isPhoto = mark.type === "photo";
  const displayCaption = mark.caption || (markerState === "generated" ? "白板记录了项目交付时间" : "");
  const isEditing = fileDetailMarkerEditor?.markId === mark.id;
  const editDraft = fileDetailMarkerEditor?.draft || "";
  return `
    <article class="file-marker-item" data-marker-id="${h(mark.id)}">
      <div class="file-marker-anchor">
        <button class="file-marker-play" data-action="file-detail-seek-marker" data-marker-id="${h(mark.id)}" aria-label="播放 ${h(mark.time)} 的标记">
          <img src="./assets/icons/play-fill.svg" alt="">
        </button>
        <button class="file-marker-time" data-action="file-detail-seek-marker" data-marker-id="${h(mark.id)}">${h(mark.time)}</button>
        <span class="file-marker-source">${h(mark.source || "APP")}</span>
      </div>
      ${isPhoto ? `
        <div class="file-marker-photo-wrap">
          <button class="file-marker-photo" data-action="file-detail-open-photo" data-marker-id="${h(mark.id)}" aria-label="打开 ${h(mark.time)} 的照片">
            <img src="${h(mark.photo || recordingPhotoAsset)}" alt="会议笔记照片">
          </button>
          <button class="file-marker-photo-delete" data-action="file-detail-delete-photo" data-marker-id="${h(mark.id)}" aria-label="删除照片标记">
            <img src="./assets/icons/trash3.svg" alt="">
          </button>
        </div>
        ${isEditing ? renderFileDetailInlineMarkerEditor(mark, "photo", editDraft) : `<button class="file-marker-copy photo-caption ${displayCaption ? "filled" : "empty"}" data-action="file-detail-edit-marker" data-marker-id="${h(mark.id)}">
          ${h(displayCaption || "添加照片说明（可选）")}
        </button>`}
      ` : `
        ${isEditing ? renderFileDetailInlineMarkerEditor(mark, "text", editDraft) : `<button class="file-marker-copy ${mark.text ? "" : "empty"}" data-action="file-detail-edit-marker" data-marker-id="${h(mark.id)}">
          ${h(mark.text || "添加标记说明")}
        </button>`}
      `}
    </article>
...
function renderFileDetailInlineMarkerEditor(mark, type, draft) {
  return `
    <div class="file-inline-edit file-inline-marker-edit ${type === "photo" ? "photo-caption" : ""}">
      <textarea id="fileDetailMarkerInput" rows="${type === "photo" ? 2 : 3}" aria-label="编辑标记内容">${h(draft)}</textarea>
    </div>
    ${renderFileDetailKeyboard(false)}
  `;
}

function renderFileDetailMarkers(markerState = "pending") {
  const markerStatus = markerState === "generating"
    ? "本次生成已读取，后续修改将在下次生成时使用"
    : "将在生成时作为辅助笔记";
  return `
    <section class="file-markers-panel" aria-label="文件标记">
      ${markerState === "generated" ? "" : `<header class="file-markers-head">
        <div>
          <p>${recordingMarks.length} 条标记 · ${markerStatus}</p>
        </div>
      </header>`}
      <div class="file-marker-list">
        ${recordingMarks.map((mark) => renderFileDetailMarkerItem(mark, markerState)).join("") || `<p class="file-marker-empty">本次录音没有标记。</p>`}
      </div>
    </section>
  `;
}

function renderFileDetailEditableContent(kind, content, options = {}) {
  const key = options.key || kind;
  const isEditing = fileDetailContentEditor?.key === key;
  const label = options.label || (kind === "summary" ? "总结" : "转写内容");
  if (!isEditing) {
    return `<button class="file-inline-content" data-action="file-detail-edit-content" data-key="${h(key)}" data-kind="${h(kind)}" aria-label="编辑${h(label)}">${h(content)}</button>`;
  }
  return `
    <div class="file-inline-edit file-inline-content-edit">
      <textarea id="fileDetailContentInput" rows="${options.rows || 5}" aria-label="编辑${h(label)}">${h(fileDetailContentEditor.draft)}</textarea>
    </div>
    ${renderFileDetailKeyboard(true)}
  `;
}

function fileDetailFormatIcon(name, label = "") {
  return `<img src="./assets/editor-icons/${name}.svg" alt="${h(label)}">`;
}

function fileDetailFormatButton({ action = "file-detail-format", value = "", label = "", icon = "", text = "", active = false, disabled = false, extraClass = "" }) {
  return `<button class="${active ? "active" : ""} ${extraClass}" data-action="${action}" data-format="${h(value)}" aria-label="${h(label)}" ${disabled ? "disabled" : ""}>
    ${icon ? fileDetailFormatIcon(icon) : text}
  </button>`;
}

function renderFileDetailFormatToolbar() {
  const keyboardButton = fileDetailFormatButton({ action: "file-detail-hide-keyboard", label: "收起键盘", icon: "keyboard-hide", extraClass: "file-format-keyboard-hide" });
  if (fileDetailFormatPanel === "heading") {
    const headingOptions = [
      ["body", "T"], ["h1", "H₁"], ["h2", "H₂"], ["h3", "H₃"], ["h4", "H₄"]
    ];
    return `<div class="file-format-toolbar is-submenu" role="toolbar" aria-label="标题级别">
      ${fileDetailFormatButton({ action: "file-detail-format-back", label: "返回格式工具栏", icon: "chevrons-left", extraClass: "file-format-back" })}
      ${headingOptions.map(([value, text]) => fileDetailFormatButton({ action: "file-detail-set-block", value, text, label: value === "body" ? "正文" : `${value.slice(1)}级标题`, active: fileDetailFormatState.block === value })).join("")}
      ${keyboardButton}
    </div>`;
  }
  if (fileDetailFormatPanel === "text") {
    const inlineOptions = [
      ["bold", "bold", "加粗"], ["italic", "italic", "斜体"], ["underline", "underline", "下划线"], ["strike", "strikethrough", "删除线"], ["code", "code", "行内代码"]
    ];
    return `<div class="file-format-toolbar is-submenu" role="toolbar" aria-label="文字样式">
      ${fileDetailFormatButton({ action: "file-detail-format-back", label: "返回格式工具栏", icon: "chevrons-left", extraClass: "file-format-back" })}
      ${inlineOptions.map(([value, icon, label]) => fileDetailFormatButton({ action: "file-detail-toggle-inline", value, icon, label, active: fileDetailFormatState[value] })).join("")}
      ${keyboardButton}
    </div>`;
  }
  if (fileDetailFormatPanel === "list") {
    const listOptions = [
      ["bullet", "list-bullet", "无序列表"], ["check", "list-check", "待办列表"], ["ordered", "list-ordered", "有序列表"]
    ];
    return `<div class="file-format-toolbar is-submenu list-submenu" role="toolbar" aria-label="列表类型">
      ${fileDetailFormatButton({ action: "file-detail-format-back", label: "返回格式工具栏", icon: "chevrons-left", extraClass: "file-format-back" })}
      ${listOptions.map(([value, icon, label]) => fileDetailFormatButton({ action: "file-detail-set-list", value, icon, label, active: fileDetailFormatState.list === value })).join("")}
      ${keyboardButton}
    </div>`;
  }

  const canOutdent = fileDetailFormatState.list !== "none" && fileDetailFormatState.indent > 0;
  const canIndent = fileDetailFormatState.list !== "none" && fileDetailFormatState.indent < 2;
  const canUndo = fileDetailEditHistory.length > 1;
  const canRedo = fileDetailEditRedo.length > 0;
  return `<div class="file-format-toolbar" role="toolbar" aria-label="文字格式">
    ${fileDetailFormatButton({ action: "file-detail-open-format-panel", value: "heading", text: ({ body: "H₁", h1: "H₁", h2: "H₂", h3: "H₃", h4: "H₄" })[fileDetailFormatState.block] || "H₁", label: "标题级别", active: fileDetailFormatState.block !== "body", extraClass: "type-tool" })}
    ${fileDetailFormatButton({ action: "file-detail-open-format-panel", value: "text", text: "Aa", label: "文字样式", active: fileDetailFormatState.bold || fileDetailFormatState.italic || fileDetailFormatState.underline || fileDetailFormatState.strike || fileDetailFormatState.code, extraClass: "type-tool" })}
    ${fileDetailFormatButton({ action: "file-detail-open-format-panel", value: "list", icon: "list-bullet", label: "列表类型", active: fileDetailFormatState.list !== "none" })}
    ${fileDetailFormatButton({ value: "outdent", icon: "outdent", label: "减少缩进", disabled: !canOutdent })}
    ${fileDetailFormatButton({ value: "indent", icon: "indent", label: "增加缩进", disabled: !canIndent })}
    ${fileDetailFormatButton({ value: "quote", icon: "quote", label: "引用", active: fileDetailFormatState.quote })}
    ${fileDetailFormatButton({ value: "undo", icon: "undo", label: "撤销", disabled: !canUndo })}
    ${fileDetailFormatButton({ value: "redo", icon: "redo", label: "重做", disabled: !canRedo })}
    ${keyboardButton}
  </div>`;
}

function renderFileDetailKeyboard(showFormatting) {
  const keys = [",", "@#", "ABC", "DEF", "⌫", "。", "GHI", "JKL", "MNO", "换行", "?", "PQRS", "TUV", "WXYZ", "重输", "符号", "123", "空格", "中英"];
  return `<section class="file-detail-edit-dock ${showFormatting ? "rich" : "plain"}" aria-label="系统输入键盘">
    ${showFormatting ? renderFileDetailFormatToolbar() : ""}
    <div class="file-edit-keyboard-accessory">
      <button data-action="file-detail-keyboard-key" data-key="command" aria-label="命令" class="key-command">⌘</button>
      <button data-action="file-detail-keyboard-key" data-key="scan" aria-label="扫描文本">${fileDetailFormatIcon("scan")}</button>
      <button data-action="file-detail-keyboard-key" data-key="cut" aria-label="剪切">${fileDetailFormatIcon("cut")}</button>
      <button data-action="file-detail-keyboard-key" data-key="emoji" aria-label="表情">${fileDetailFormatIcon("smile")}</button>
      <button data-action="file-detail-keyboard-key" data-key="assistant" aria-label="智能输入">${fileDetailFormatIcon("assistant")}</button>
      <b>${fileDetailFormatIcon("microphone")}<span>点击说话</span></b>
      <button data-action="file-detail-hide-keyboard" aria-label="收起键盘">${fileDetailFormatIcon("chevron-down")}</button>
    </div>
    <div class="file-edit-keyboard-grid">
      ${keys.map((key) => `<button data-action="file-detail-keyboard-key" data-key="${h(key)}" class="${[",", "⌫", "。", "换行", "?", "重输", "符号"].includes(key) ? "modifier" : ""} ${key === "空格" ? "space" : ""}">${h(key)}</button>`).join("")}
    </div>
  </section>`;
}

function persistFileDetailMarkerEdit(showNotice = false) {
  if (!fileDetailMarkerEditor) return;
  const draft = fileDetailMarkerEditor.draft.trim();
  recordingMarks = recordingMarks.map((mark) => {
    if (mark.id !== fileDetailMarkerEditor.markId) return mark;
    return mark.type === "photo"
      ? { ...mark, caption: draft, captionSource: draft ? "user" : "" }
      : { ...mark, text: draft };
  });
  fileDetailMarkerEditor = null;
  if (showNotice) showFileDetailNotice(draft ? "标记已自动保存" : "标记文字已清空，时间锚点已保留");
}

function persistFileDetailContentEdit(showNotice = false) {
  if (!fileDetailContentEditor) return;
  const draft = fileDetailContentEditor.draft.trim();
  if (draft) {
    if (fileDetailContentEditor.kind === "summary") fileDetailContent.summary = draft;
    if (fileDetailContentEditor.kind === "transcript") {
      const index = Number(fileDetailContentEditor.key.split("-")[1]);
      if (fileDetailContent.transcript[index]) fileDetailContent.transcript[index].text = draft;
    }
  }
  fileDetailContentEditor = null;
  if (showNotice && draft) showFileDetailNotice("内容已自动保存");
}

function activeFileDetailInput() {
  return document.getElementById(fileDetailMarkerEditor ? "fileDetailMarkerInput" : "fileDetailContentInput");
}

function defaultFileDetailFormatState() {
  return {
    block: "body",
    bold: false,
    italic: false,
...
function storeFileDetailFormatState() {
  if (!fileDetailContentEditor?.key) return;
  fileDetailContentFormats[fileDetailContentEditor.key] = { ...fileDetailFormatState };
}

function pushFileDetailEditHistory(value) {
  if (!fileDetailContentEditor || fileDetailEditHistory.at(-1) === value) return;
  fileDetailEditHistory.push(value);
  if (fileDetailEditHistory.length > 40) fileDetailEditHistory.shift();
  fileDetailEditRedo = [];
}

function applyFileDetailFormatState() {
  const input = activeFileDetailInput();
  if (!input || !fileDetailContentEditor) return;
  ["body", "h1", "h2", "h3", "h4"].forEach((value) => input.classList.toggle(`is-${value}`, fileDetailFormatState.block === value));
  input.classList.toggle("is-bold", fileDetailFormatState.bold);
  input.classList.toggle("is-italic", fileDetailFormatState.italic);
  input.classList.toggle("is-underline", fileDetailFormatState.underline);
  input.classList.toggle("is-strike", fileDetailFormatState.strike);
  input.classList.toggle("is-code", fileDetailFormatState.code);
  input.classList.toggle("is-list", fileDetailFormatState.list !== "none");
  input.classList.toggle("is-quote", fileDetailFormatState.quote);
  input.dataset.list = fileDetailFormatState.list;
  input.dataset.indent = String(fileDetailFormatState.indent);
}

function focusFileDetailEditor(inputId) {
  requestAnimationFrame(() => {
    ...
    applyFileDetailFormatState();
  });
}

function rerenderFileDetailFormatEditor() {
  render();
  focusFileDetailEditor("fileDetailContentInput");
}

function renderFileDetailPhotoViewer() {
  if (!fileDetailPhotoViewerId) return "";
  const mark = recordingMarks.find((item) => item.id === fileDetailPhotoViewerId && item.type === "photo");
  if (!mark) return "";
  return `
    <section class="file-marker-viewer" aria-label="照片预览">
      <button class="file-marker-viewer-close" data-action="file-detail-close-photo" aria-label="关闭照片">×</button>
      <img src="${h(mark.photo || recordingPhotoAsset)}" alt="会议笔记照片大图">
      <p>${h(mark.time)} · ${h(mark.source || "APP")}</p>
      <strong>${h(mark.caption || "尚未添加照片说明")}</strong>
    </section>
  `;
}

function renderFileDetailTabs(state = "generated") {
  const tabs = state === "generated"
    ? [["标记", "markers"], ["总结", "summary"], ["转写记录", "transcript"]]
    : [["标记", "markers"], ["总结", "summary"]];
  return `
    <nav class="file-detail-tabs ${state === "generated" ? "is-generated" : "is-pending"}" aria-label="文件详情一级导航">
      ${tabs.map(([label, value]) => `<button class="${fileDetailActiveTab === value ? "active" : ""}" data-action="file-detail-set-tab" data-value="${value}">${h(label)}</button>`).join("")}
    </nav>
  `;
}

function renderFileDetailSummaryTabs() {
  const tabs = [["智能总结", "summary"], ["章节大纲", "outline"], ["待办事项", "todos"], ["思维导图", "mindmap"]];
  return `
    <nav class="file-detail-summary-tabs" aria-label="总结二级导航">
      ${tabs.map(([label, value]) => `<button class="${fileDetailSummaryTab === value ? "active" : ""}" data-action="file-detail-set-summary-tab" data-value="${value}">${h(label)}</button>`).join("")}
    </nav>
  `;
}

function renderFileDetailUntranscribedSummary() {
  return `
    <section class="untranscribed-summary" aria-label="未转写总结">
      <span class="untranscribed-summary-icon" aria-hidden="true"></span>
      <h2>未转写总结</h2>
      <p>完成转写并生成后，将在这里展示录音总结。</p>
      <p>已保存的 ${recordingMarks.length} 条标记会作为生成辅助笔记。</p>
    </section>
  `;
}

function renderFileDetailSummaryContent(editingSection) {
  return `
    <div class="file-detail-summary-long-page">
      <section class="file-detail-section ai-section ${editingSection === "summary" ? "is-editing-section" : ""}" data-summary-section="summary" id="detail-summary">
        <h2>智能总结</h2>
        ${renderFileDetailEditableContent("summary", fileDetailContent.summary, { label: "智能总结", rows: 6 })}
        <h3>关键结论</h3>
        <ul class="summary-list summary-key-points">
          <li><strong>产品目标：</strong>提升拼团活动创建效率与数据转化效果，支撑多场景营销。</li>
          <li><strong>核心功能：</strong>活动创建与配置、团购规则设置、参与流程、数据看板与导出。</li>
          <li><strong>集成方案：</strong>对接用户中心、订单中心与消息推送服务。</li>
          <li><strong>范围约定：</strong>本期不包含社交裂变玩法与优惠券联动。</li>
          <li><strong>里程碑：</strong>原型评审完成后进入开发与灰度测试。</li>
        </ul>
        <h3>下一步</h3>
        <ul class="summary-list summary-next-steps">
          <li>产品：输出 PRD 与原型评审最终版。</li>
          <li>研发：细化技术方案与接口文档。</li>
          <li>设计：补充关键页面视觉稿。</li>
          <li>运营：提供活动规则示例与文案支持。</li>
        </ul>
      </section>
      <section class="file-detail-section ai-section outline-section" data-summary-section="outline" id="detail-outline">
      ...
      <section class="file-detail-section ai-section" data-summary-section="todos" id="detail-todos">
        <h2>待办事项</h2>
      ...
      <section class="file-detail-section ai-section mindmap-section" data-summary-section="mindmap" id="detail-mindmap">
...
function renderFileDetailTranscriptItem(item, index) {
  ...
  return `
    <article class="transcript-line" data-transcript-index="${index}">
      ...
      ${renderFileDetailEditableContent("transcript", text, { key: `transcript-${index}`, label: `${time} ${speaker} 转写内容`, rows: 4 })}
    </article>
  `;
}

function updateFileDetailSummaryTab(target) {
  const allowed = ["summary", "outline", "todos", "mindmap"];
  fileDetailSummaryTab = allowed.includes(target) ? target : "summary";
  document.querySelectorAll(".file-detail-summary-tabs [data-action='file-detail-set-summary-tab']").forEach((button) => {
    button.classList.toggle("active", button.dataset.value === fileDetailSummaryTab);
  });
}

function scrollFileDetailSummaryToSection(target, behavior = "smooth") {
  const scroller = document.querySelector(".file-detail-page");
  const primaryTabs = document.querySelector(".file-detail-tabs");
  const summaryTabs = document.querySelector(".file-detail-summary-tabs");
  const section = document.getElementById(`detail-${target}`);
  if (!scroller || !section) return;
  const scrollerRect = scroller.getBoundingClientRect();
  const sectionRect = section.getBoundingClientRect();
  const stickyOffset = (primaryTabs?.offsetHeight || 0) + (summaryTabs?.offsetHeight || 0) - 8;
  const nextTop = scroller.scrollTop + sectionRect.top - scrollerRect.top - stickyOffset;
  updateFileDetailSummaryTab(target);
  scroller.scrollTo({ top: Math.max(0, nextTop), behavior });
}

function setupFileDetailSummarySectionSpy() {
  const scroller = document.querySelector(".file-detail-page");
  const sections = [...document.querySelectorAll("[data-summary-section]")];
  if (!scroller || !sections.length || fileDetailActiveTab !== "summary") return;
  const update = () => {
    const primaryTabs = document.querySelector(".file-detail-tabs");
    const summaryTabs = document.querySelector(".file-detail-summary-tabs");
    const scrollerRect = scroller.getBoundingClientRect();
    const threshold = scrollerRect.top + (primaryTabs?.offsetHeight || 0) + (summaryTabs?.offsetHeight || 0) + 20;
    let active = sections[0].dataset.summarySection;
    sections.forEach((section) => {
      if (section.getBoundingClientRect().top <= threshold) active = section.dataset.summarySection;
    });
    updateFileDetailSummaryTab(active);
  };
...
function renderFileDetail() {
  const activeTab = ["markers", "summary", "transcript"].includes(fileDetailActiveTab) ? fileDetailActiveTab : "summary";
  fileDetailActiveTab = activeTab;
  fileDetailSummaryTab = ["summary", "outline", "todos", "mindmap"].includes(fileDetailSummaryTab) ? fileDetailSummaryTab : "summary";
  const contentEditing = Boolean(fileDetailContentEditor);
  const editingSection = fileDetailContentEditor?.kind === "summary"
    ? "summary"
    : fileDetailContentEditor?.kind === "transcript"
      ? "transcript"
      : "";
  return `
    <div class="statusbar detail-statusbar"><span>9:41</span><span>5G</span><span>82%</span></div>
    <main class="file-detail-page ${fileDetailMarkerEditor || contentEditing ? "is-editing" : ""} ${contentEditing ? "is-rich-editing" : ""}">
      <div class="detail-toolbar">
        <button class="round-btn" data-go="APP-HOME-01" aria-label="返回">‹</button>
        <h1 class="detail-toolbar-title">拼团营销工具需求评审会</h1>
      ...
      ${renderFileDetailPlayer("00:43:52")}
      ${renderFileDetailTabs("generated")}
      <div class="file-detail-view" aria-label="文件详情内容">
        ${activeTab === "markers" ? `
          <section class="file-detail-section ai-section" data-detail-section="markers" id="detail-markers">
            ${renderFileDetailMarkers("generated")}
          </section>
        ` : activeTab === "transcript" ? `
          <section class="file-detail-section ai-section ${editingSection === "transcript" ? "is-editing-section" : ""}" data-detail-section="transcript" id="detail-transcript">
            ${fileDetailContent.transcript.map((item, index) => renderFileDetailTranscriptItem(item, index)).join("")}
          </section>
        ` : `
          ${renderFileDetailSummaryTabs()}
          ${renderFileDetailSummaryContent(editingSection)}
        `}
      </div>
    </main>
    ${contentEditing ? "" : `<div class="note-question-bar" data-action="open-ask-plaud"><span>Beta</span><input placeholder="对此笔记提问" aria-label="对此笔记提问" readonly></div>`}
    ${renderAskPlaudSheet()}
    ${renderFileDetailPhotoViewer()}
    ${fileDetailNotice ? `<div class="file-detail-notice" role="status">${h(fileDetailNotice)}</div>` : ""}
    ${moveFolderToast ? `<div class="toast-message">文件夹已移动至${h(moveFolderToast)}</div>` : ""}
  `;
}

function renderFileDetailEmpty() {
  const activeTab = ["markers", "summary"].includes(fileDetailActiveTab) ? fileDetailActiveTab : "markers";
  fileDetailActiveTab = activeTab;
  return `
    <div class="statusbar detail-statusbar"><span>9:41</span><span>5G</span><span>82%</span></div>
    <main class="file-detail-page empty-detail-page ${fileDetailMarkerEditor ? "is-editing" : ""}">
      <div class="detail-toolbar">
        <button class="round-btn" data-go="APP-HOME-01" aria-label="返回">‹</button>
        <h1 class="detail-toolbar-title">拼团营销工具需求评审会</h1>
        <div class="detail-actions">
      ...
      ${renderFileDetailPlayer("00:43:52")}
      ${renderFileDetailTabs("pending")}
      ${activeTab === "markers" ? renderFileDetailMarkers("pending") : renderFileDetailUntranscribedSummary()}
    </main>
    <div class="empty-generate-bar">
      <button class="primary-btn" data-go="APP-AI-01">生成</button>
    </div>
    ${renderFileDetailPhotoViewer()}
    ${fileDetailNotice ? `<div class="file-detail-notice" role="status">${h(fileDetailNotice)}</div>` : ""}
  `;
...
function renderFileDetailGenerating() {
  if (!fileDetailGeneratingInitialized) {
    fileDetailActiveTab = "summary";
    fileDetailGeneratingInitialized = true;
  }
  const activeTab = ["markers", "summary"].includes(fileDetailActiveTab) ? fileDetailActiveTab : "summary";
  fileDetailActiveTab = activeTab;
  return `
    <div class="statusbar detail-statusbar"><span>9:41</span><span>5G</span><span>82%</span></div>
    <main class="file-detail-page generating-detail-page ${fileDetailMarkerEditor ? "is-editing" : ""}">
      <div class="detail-toolbar">
        <button class="round-btn" data-go="APP-HOME-01" aria-label="返回">‹</button>
        <h1 class="detail-toolbar-title">拼团营销工具需求评审会</h1>
        <div class="detail-actions">
      ...
      ${renderFileDetailPlayer("00:43:52")}
      ${renderFileDetailTabs("generating")}
      ${activeTab === "markers" ? renderFileDetailMarkers("generating") : `<section class="generation-card">
...
function renderShareSheet() {
  return `
    ...
    <section class="share-sheet">
      ...
      <button class="share-row"><span class="share-icon doc"></span><b>标记</b><i>□</i></button>
      <button class="share-row"><span class="share-icon doc"></span><b>总结</b><i>□</i></button>
      <h3>导出文件</h3>
      <button class="share-row" data-go="APP-FILE-24"><span class="share-icon audio"></span><b>录音</b><i>›</i></button>
      <button class="share-row" data-go="APP-FILE-25"><span class="share-icon doc"></span><b>转写</b><i>›</i></button>
      <button class="share-row" data-go="APP-FILE-26"><span class="share-icon mark"></span><b>标记</b><i>›</i></button>
      <button class="share-row" data-go="APP-FILE-27"><span class="share-icon doc"></span><b>总结</b><i>›</i></button>
      <button class="share-row" data-go="APP-FILE-28"><span class="share-icon map"></span><b>思维导图</b><i>›</i></button>
    </section>
  `;
}

const exportSheetConfigs = {
  audio: { pageId: "APP-FILE-24", title: "录音", formats: ["MP3", "WAV"] },
  transcript: {
    pageId: "APP-FILE-25",
    title: "转写",
    sectionTitle: "包含",
    formats: ["TXT", "SRT", "DOCX", "PDF"],
    options: [
      { key: "includeTimestamps", label: "时间戳", control: "switch" },
      { key: "includeSpeakers", label: "自动标注说话人", control: "switch" }
    ]
  },
  marks: {
    pageId: "APP-FILE-26",
    ...
    sectionTitle: "包含",
    formats: ["DOCX", "PDF"],
    options: [{ key: "includeTimestamps", label: "时间戳", control: "switch" }]
  },
  summary: {
    pageId: "APP-FILE-27",
    title: "总结",
    formats: ["TXT", "Markdown", "DOCX", "PDF"],
    options: [{ key: "summaryOnly", label: "总结", control: "radio" }]
  },
  mindmap: {
    pageId: "APP-FILE-28",
    title: "思维导图",
    formats: ["JPEG", "Markdown"],
    options: [{ key: "summaryStructure", label: "总结", control: "radio" }]
  }
};

function renderExportSheet(type) {
  const config = exportSheetConfigs[type];
  const state = exportSheetState[type];
  const hasOptions = Boolean(config.options?.length);
  const formatMenuOpen = activeExportFormatMenu === type;
  return `
    ${renderFileDetail()}
    <div class="scrim light" data-action="close-export-sheet"></div>
    <section class="export-sheet" aria-label="导出${h(config.title)}">
      <div class="export-sheet-head">
        <h2>导出${h(config.title)}</h2>
        <button class="export-close" data-action="close-export-sheet" aria-label="关闭">×</button>
      </div>
      <div class="export-divider"></div>
      ${hasOptions ? `
        ${config.sectionTitle ? `<h3>${h(config.sectionTitle)}</h3>` : ""}
        <div class="export-option-list">
          ${config.options.map((option) => {
            const selected = option.control === "radio" || Boolean(state[option.key]);
            return `<button class="export-option" ${option.control === "switch" ? `data-action="toggle-export-option" data-export-type="${type}" data-export-option="${option.key}" aria-pressed="${selected}"` : "disabled"}>
              <span>${h(option.label)}</span>
              <i class="${option.control === "switch" ? `export-switch ${selected ? "is-on" : ""}` : "export-radio is-selected"}" aria-hidden="true"></i>
            </button>`;
          }).join("")}
        </div>
        <div class="export-divider section-divider"></div>
      ` : ""}
      <button class="export-format-row" data-action="toggle-export-format-menu" data-export-type="${type}" aria-expanded="${formatMenuOpen}">
        <span>导出格式</span><b>${h(state.format)}</b><i aria-hidden="true">⌄</i>
      </button>
      ${formatMenuOpen ? `<div class="export-format-menu" role="listbox" aria-label="选择导出格式">
        ${config.formats.map((format) => `<button data-action="select-export-format" data-export-type="${type}" data-export-format="${format}" role="option" aria-selected="${state.format === format}"><span>${h(format)}</span><i>${state.format === format ? "◉" : "○"}</i></button>`).join("")}
      </div>` : ""}
      ${exportNotice ? `<p class="export-notice" role="status">${h(exportNotice)}</p>` : ""}
      <button class="export-submit" data-action="submit-export" data-export-type="${type}">导出</button>
    </section>
  `;
}

function closeExportSheet() {
  activeExportFormatMenu = "";
  exportNotice = "";
  go("APP-FILE-06");
}
...
function renderShareControls() {
  const selectedLabels = [
    ...
    ["marks", "标记"],
    ["summary", "总结"]
  ].filter(([key]) => sharePermissionState[key]).map(([, label]) => label);
  const includeLabel = selectedLabels.length === 4 ? "全部内容" : selectedLabels.length ? selectedLabels.join("、") : "未选择内容";
  return `
    <button class="include-row" data-go="APP-FILE-09"><span>选择包含内容</span><b>${includeLabel} ›</b></button>
...
function renderGenerationSettingRow(label, value, opts = {}) {
  const attrs = opts.action
    ? ` data-action="${h(opts.action)}"`
    : opts.go ? ` data-go="${h(opts.go)}"` : "";
  const tail = opts.toggle
    ? `<span class="toggle ${generationSpeakerDiarization ? "" : "off"}"></span>`
    : `<span class="generation-setting-value">${h(value)}${opts.required ? '<b class="required-dot" aria-label="必选"></b>' : ""}<i>›</i></span>`;
  return `<button class="generation-setting-row${opts.required ? " required" : ""}"${attrs}><strong>${h(label)}</strong>${tail}</button>`;
}
...
function renderGenerate() {
  ...
  return `
    ${renderFileDetail()}
    <div class="scrim generate-sheet-scrim" data-go="APP-FILE-01"></div>
    <section class="bottom-sheet generate-sheet">
      <div class="handle"></div>
      <div class="generate-sheet-title"><h2>转写与生成</h2><span>01:12:17</span></div>
      <button class="option ${generationMode === "auto" ? "selected" : ""}" data-mode="auto">
        <span class="option-dot">${generationMode === "auto" ? "✓" : ""}</span>
        <div><strong>自动生成</strong><p class="caption">自动生成转写与总结，无需手动设置</p></div>
      </button>
      <button class="option ${generationMode === "custom" ? "selected" : ""}" data-mode="custom">
        <span class="option-dot">${generationMode === "custom" ? "✓" : ""}</span>
        <div><strong>自定义生成</strong><p class="caption">可调整模板、说话人、语言和 AI 模型</p></div>
      </button>
      ${generationMode === "custom" ? `
        <div class="generation-settings">
          ${renderGenerationSettingRow("模板", selectedTemplateLabel() || "查看全部", { action: "open-template-library", required: !selectedTemplateId })}
          ${renderGenerationSettingRow("自动标注说话人", "", { action: "toggle-generation-speakers", toggle: true })}
          ${renderGenerationSettingRow("音频语言", selectedAudioLanguage, { go: "APP-AI-02" })}
          ${renderGenerationSettingRow("AI 模型", selectedAiModel, { go: "APP-AI-03" })}
        </div>
      ` : ""}
      <section class="generation-usage">
        <h3>用量确认</h3>
        <div>
          <span><small>预计消耗</small><b>73 <em>分钟</em></b></span>
          <span><small>剩余额度</small><b>420 <em>分钟</em></b></span>
        </div>
      </section>
      ${quotaInsufficient ? `
      ...
      ` : ""}
      <button class="primary-btn ${quotaInsufficient ? "disabled" : ""}" ${quotaInsufficient ? "disabled" : 'data-action="submit-generation"'}>立即生成</button>
      ${generationNotice ? `<div class="generation-template-toast" role="status" aria-live="polite">${h(generationNotice)}</div>` : ""}
    </section>
  `;
}

function renderSelectionChoice(title, desc, selected, action, value, premium = false) {
  return `
    <button class="selection-choice" data-action="${h(action)}" data-value="${h(value)}">
      <span><strong>${h(title)}${premium ? '<i class="premium-dot"></i>' : ""}</strong>${desc ? `<small>${h(desc)}</small>` : ""}</span>
      ${selected ? "<b>✓</b>" : ""}
    </button>
  `;
}

function renderAudioLanguageSheet() {
  const languages = [
    ["中文简体 - 普通话", "中文（简体 - 普通话）"],
    ["English (US)", "英语（美国）"],
    ["日本語", "日语"],
  ...
  return `
    ${renderFileDetail()}
    <div class="scrim" data-go="APP-AI-01"></div>
    <section class="bottom-sheet ai-selection-sheet">
      <header><h2>音频语言</h2><button data-go="APP-AI-01" aria-label="关闭">×</button></header>
      <div class="selection-divider"></div>
      <h3>最近使用</h3>
      ${renderSelectionChoice("自动", "系统会自动识别录音语言", selectedAudioLanguage === "自动", "select-ai-language", "自动")}
      <h3>热门语言</h3>
      ${languages.map(([title, desc]) => renderSelectionChoice(title, desc, selectedAudioLanguage === title, "select-ai-language", title)).join("")}
    </section>
  `;
}

function renderAiModelSheet() {
  const models = [
    ["GPT-5", "均衡稳健，适合多数会议与访谈", false],
    ["GPT-5.5", "推理更深入，适合复杂讨论与结构化笔记", false],
    ["Gemini 2.5 Pro", "适合长录音与复杂内容分析", false],
    ["Gemini 3.5 Flash", "响应迅速，适合快速生成要点", true],
    ["Claude Sonnet 5", "理解准确，适合忠实整理讨论内容", true],
    ["Claude Opus 4.8", "适合深度推理与复杂洞察", true]
  ];
  return `
    ${renderFileDetail()}
    <div class="scrim" data-go="APP-AI-01"></div>
    <section class="bottom-sheet ai-selection-sheet model-selection-sheet">
      <header><h2>AI 模型</h2><button data-go="APP-AI-01" aria-label="关闭">×</button></header>
      <div class="selection-divider"></div>
      ${renderSelectionChoice("自动", "智能匹配适合的模型", selectedAiModel === "自动", "select-ai-model", "自动")}
      ${models.map(([title, desc, premium]) => renderSelectionChoice(title, desc, selectedAiModel === title, "select-ai-model", title, premium)).join("")}
    </section>
  `;
}
...
function meTop(title, opts = {}) {
  const right = opts.action
    ? `<button class="top-action" data-action="${opts.action}">${h(opts.actionLabel || "添加")}</button>`
...
function recordingMinutesForDate(date, index) {
  const seed = (date.getFullYear() * 17 + (date.getMonth() + 1) * 31 + date.getDate() * 13 + index) % 29;
  if (seed < 16) return 0;
  if (seed < 21) return 8 + (seed * 3) % 18;
  if (seed < 26) return 31 + (seed * 7) % 54;
  return 96 + (seed * 11) % 88;
}
...
function heatmapDots() {
  ...
  for (let index = 0; index < totalDays; index += 1) {
    ...
    const minutes = recordingMinutesForDate(date, index);
    const level = minutes === 0 ? 0 : Math.min(4, Math.ceil(minutes / 45));
    const label = `${dateKey} · 录音卡实际录音 ${minutes} 分钟`;
    cells.push(`<i class="level-${level}" role="gridcell" aria-rowindex="${index % daysPerWeek + 1}" aria-colindex="${Math.floor(index / daysPerWeek) + 1}" aria-label="${label}" title="${label}" data-date="${dateKey}" data-recording-minutes="${minutes}"></i>`);
  }
  ...
  for (let w = 0; w < weekCount; w += 1) {
    ...
    day.setDate(start.getDate() + w * daysPerWeek);
    const m = day.getMonth();
    if (m !== lastMonth) {
      monthSpans.push(`<span style="grid-column: ${w + 1} / span 4;">${monthNames[m]}</span>`);
      lastMonth = m;
    }
  }

  return `
    <section class="settings-week-heatmap" aria-label="录音卡实际录音使用热力图">
      <div class="settings-week-heatmap-scroll" data-visible-days="180" tabindex="0" aria-label="录音使用热力图，可横向滚动查看更早日期">
        <div class="settings-week-heatmap-container" style="--settings-week-count:${weekCount}">
          <div class="settings-week-heatmap-grid" role="grid" aria-rowcount="7" aria-colcount="${weekCount}">${cells.join("")}</div>
          <div class="settings-week-heatmap-months" aria-hidden="true">${monthSpans.join("")}</div>
        </div>
      </div>
    </section>
  `;
...
function renderAccountSettingsHome() {
  return `
    <main class="me-simple-page account-settings-home">
      ${meTop("设置")}
      <section class="account-profile settings-hero">
      ...
      <section class="account-settings-list">
        ${accountRow("偏好设置", { icon: "sliders", go: "APP-ME-26" })}
...
function prefsRow(label, opts = {}) {
  const attrs = [
    opts.go ? `data-go="${opts.go}"` : "",
    opts.action ? `data-action="${opts.action}"` : "",
    opts.valueKey !== undefined ? `data-value="${h(opts.valueKey)}"` : "",
    opts.notification ? `data-notification="${opts.notification}"` : "",
    opts.disabled ? "disabled" : ""
  ].filter(Boolean).join(" ");
  const tail = opts.toggle !== undefined
    ? togglePill(opts.toggle, opts.disabled)
    : opts.check ? "<b>✓</b>" : (opts.arrow === false ? "<span></span>" : "<i>›</i>");
  return `
    <button class="prefs-row ${opts.danger ? "danger" : ""} ${opts.disabled ? "disabled" : ""}" ${attrs}>
      <span><b>${h(label)}</b>${opts.note ? `<small>${h(opts.note)}</small>` : ""}</span>
      ${opts.value ? `<em>${h(opts.value)}</em>` : ""}
      ${tail}
    </button>
  `;
...
function renderPreferencesBase() {
  return `
    <main class="me-simple-page prefs-page">
      ${meTop("偏好设置", { back: "APP-ME-16" })}
      <section class="settings-hero">
        <h1>让每次录音更贴合你的习惯</h1>
        <p>转写偏好只影响后续录音，不会修改已有文件。</p>
      </section>
      <section class="prefs-section">
        <h2>转写</h2>
        ${prefsRow("转写语言", { value: settingsState.transcriptionLanguage, go: "APP-ME-27" })}
        ${prefsRow("自动提升转写可读性", { note: "智能补全标点、分段并优化口语表达", toggle: settingsState.improveReadability, action: "toggle-readability" })}
        ${prefsRow("自动标注说话人", { value: settingsState.speakerDiarization ? "已开启" : "已关闭", go: "APP-ME-28" })}
        ${prefsRow("自定义术语", { value: customTerms.length ? `${customTerms.length} 个` : "未设置", go: "APP-ME-34" })}
      </section>
      <section class="prefs-section">
        <h2>应用</h2>
        ${prefsRow("消息与通知", { go: "APP-ME-37" })}
        ${prefsRow("应用锁", { value: settingsState.appLockEnabled ? settingsState.autoLockDuration : "已关闭", go: "APP-ME-38" })}
      </section>
      <section class="prefs-section">
        <h2>AI 与隐私</h2>
        ${prefsRow("帮助改进 AI", { note: "经你同意后使用去标识化片段改进识别效果", toggle: settingsState.improveAi, action: "toggle-improve-ai" })}
      </section>
    </main>
  `;
}
...
function renderPreferences() {
  const base = renderPreferencesBase();
  if (!settingsNotice) return base;
  return `
    ${base}
    <div class="scrim" data-action="dismiss-settings-notice"></div>
    <section class="permission-sheet">
      <button class="sheet-close" data-action="dismiss-settings-notice" aria-label="关闭">×</button>
      <h2>使用通用转写优化?</h2>
      <p>你还没有选择行业或添加常用术语。仍可开启自动提升转写可读性，系统将先使用通用词库。</p>
      <button class="dark" data-action="confirm-readability">仍要开启</button>
      <button class="outline" data-action="dismiss-settings-notice">稍后设置术语</button>
    </section>
  `;
}

function renderTranscriptionLanguage() {
  const languages = ["自动检测", "简体中文", "English", "日本語", "繁體中文"];
  return `
    <main class="me-simple-page full-list-page prefs-page">
      ${meTop("转写语言", { back: "APP-ME-26" })}
      <section class="prefs-section">
        <p>设为后续录音的默认转写语言。</p>
        ${languages.map((language) => prefsRow(language, { check: settingsState.transcriptionLanguage === language, action: "select-language", valueKey: language })).join("")}
      </section>
    </main>
  `;
}
...
function renderSpeakerSettings() {
  return `
    <main class="me-simple-page prefs-page speaker-page">
      ${meTop("自动标注说话人", { back: "APP-ME-26" })}
      <section class="settings-hero">
        <h1>让转写自动区分不同说话人</h1>
        <p>声音样本仅用于在你的录音中识别本人。</p>
      </section>
      <section class="prefs-section">
        ${prefsRow("自动标注说话人", { toggle: settingsState.speakerDiarization, action: "toggle-speaker-diarization" })}
        ${prefsRow("我的声音样本", { value: settingsState.voiceSampleCreated ? settingsState.speakerName : "未创建", go: "APP-ME-29" })}
      </section>
    </main>
  `;
}
...
function renderVoiceSamples() {
  return `
    <main class="me-simple-page voice-sample-page speaker-page">
      ${meTop("我的声音样本", { back: "APP-ME-28" })}
      <section class="settings-hero">
        <h1>${settingsState.voiceSampleCreated ? h(settingsState.speakerName) : "创建我的声音样本"}</h1>
        <p>${settingsState.voiceSampleCreated ? "样本已可用于自动识别本人。" : "录制约 15 秒清晰语音，帮助系统在多人录音中识别你。"}</p>
      </section>
      <section class="prefs-section">
        ${settingsState.voiceSampleCreated
          ? `${prefsRow("样本名称", { value: settingsState.speakerName, go: "APP-ME-30" })}${prefsRow("重新录制", { go: "APP-ME-31" })}`
          : `<button class="primary-btn" data-go="APP-ME-30">创建声音样本</button>`}
      </section>
    </main>
  `;
}
...
function renderMicrophonePermission() {
  return `
    ${renderVoiceSamples()}
    <div class="scrim" data-go="APP-ME-29"></div>
    <section class="permission-sheet">
      <button class="sheet-close" data-go="APP-ME-29" aria-label="关闭">×</button>
      <h2>允许使用麦克风</h2>
      <p>录制声音样本需要访问麦克风。样本只用于识别你的声音，你可随时重新录制。</p>
      <button class="dark" data-action="allow-microphone">允许并继续</button>
      <button class="outline" data-go="APP-ME-29">暂不允许</button>
    </section>
  `;
}
  ...
  return `
    <main class="me-simple-page voice-sample-page speaker-page">
      ...
      <section class="prefs-section">
        ...
        <p>${settingsState.voiceRecording ? "00:08 / 00:15 · 请保持自然语速" : "建议在安静环境中录制 15 秒"}</p>
        <button class="primary-btn" data-action="toggle-voice-recording">${settingsState.voiceRecording ? "完成录制" : "开始录制"}</button>
      </section>
    </main>
  `;
}

function renderVoiceSampleComplete() {
  return `
    <main class="me-simple-page voice-sample-page speaker-page">
      ${meTop("声音样本完成", { back: "APP-ME-32" })}
      <section class="settings-hero">
        <h1>声音样本已创建</h1>
        <p>之后的转写会优先把识别到的本人声音标注为“${h(settingsState.speakerName)}”。</p>
      </section>
      <section class="prefs-section">
        <button class="primary-btn" data-go="APP-ME-29">完成</button>
      </section>
    </main>
  `;
}

function renderTermsBase() {
  return `
    <main class="me-simple-page term-page prefs-page">
      ${meTop("自定义术语", { back: "APP-ME-26" })}
      <section class="settings-hero terms-overview-hero">
        <div class="terms-hero-icon" aria-hidden="true">T</div>
        <h1>自定义术语</h1>
        <span>Beta</span>
        <p>AI 将优先识别你所在行业的专有术语，并结合自定义术语，提升转写准确率。</p>
      </section>
      <section class="prefs-section">
        ${prefsRow("启用自定义术语", {
          toggle: settingsState.customTermsEnabled,
          action: "toggle-custom-terms"
        })}
      </section>
      <section class="prefs-section">
        <h2>行业与术语</h2>
        ${prefsRow("行业", { value: settingsState.industry || "未选择", go: "APP-ME-35" })}
        ${prefsRow("常用术语", { value: `${customTerms.length}`, go: "APP-ME-36" })}
      </section>
    </main>
  `;
}
...
function renderTerms() {
  const base = renderTermsBase();
  if (pendingTermsEnableNotice) {
    return `
      ${base}
      <div class="scrim" data-action="dismiss-terms-enable-notice"></div>
      <section class="permission-sheet term-enable-sheet">
        <h2>启用自定义术语</h2>
        <p>请先选择行业或添加自定义术语后，再启用该功能。</p>
        <button class="dark" data-action="dismiss-terms-enable-notice">知道了</button>
      </section>
    `;
  }
  return base;
...
function renderIndustryList() {
  const industries = ["信息技术与工程", "能源与环境", "金融与法律", "教育与研究", "公共服务", "医疗与健康", "创意与传媒", "建筑与房地产", "人力资源与行政", "零售与消费", "旅游与物流"];
  return `
    <main class="me-simple-page full-list-page term-page">
      ${meTop("行业", { back: "APP-ME-34" })}
      <section class="prefs-section">
        ${industries.map((industry) => {
          return prefsRow(industry, { check: settingsState.industry === industry, action: "select-industry", valueKey: industry });
        }).join("")}
      </section>
    </main>
  `;
...
function termGroupLabel(term) {
  const first = String(term || "").trim().charAt(0);
  return /^[A-Za-z]$/.test(first) ? first.toUpperCase() : "#";
}

function renderTermRows() {
  const groups = customTerms.reduce((acc, term, index) => {
    const key = termGroupLabel(term);
    if (!acc[key]) acc[key] = [];
    acc[key].push([term, index]);
    return acc;
  }, {});
  return Object.keys(groups).sort().map((key) => `
    <section class="terms-alpha-group">
      <h2>${h(key)}</h2>
      ${groups[key].map(([term, index]) => `
        <button class="terms-list-row" data-action="open-edit-term" data-term-index="${index}">
          <span>${h(term)}</span>
          <i>›</i>
        </button>
      `).join("")}
...
function renderTermEditorSheet() {
  if (!termSheetMode) return "";
  const editing = termSheetMode === "edit" && termEditorIndex !== null;
  const value = editing ? customTerms[termEditorIndex] || "" : "";
  return `
    <div class="scrim light" data-action="close-term-sheet"></div>
    <section class="term-editor-sheet">
      <header>
        <button data-action="close-term-sheet">取消</button>
        <button data-action="save-term-sheet">保存</button>
      </header>
      <label class="term-editor-field">
        <span>术语</span>
        <input id="termSheetInput" value="${h(value)}" maxlength="40" placeholder="AI 录音卡" aria-label="术语" autofocus>
        <button type="button" data-action="clear-term-draft" aria-label="清空术语">×</button>
      </label>
      <p class="${termSheetError ? "term-sheet-error" : ""}">${termSheetError || "添加常用术语，例如：名字、公司、行业术语等"}</p>
      ${editing ? `<button class="term-delete-button" data-action="delete-term-sheet">删除术语</button>` : ""}
    </section>
  `;
}
...
function renderCommonTerms() {
  const base = `
    <main class="me-simple-page term-page common-terms-page">
      ${meTop("常用术语", { back: "APP-ME-34", action: "open-term-sheet", actionLabel: "添加" })}
      ${customTerms.length ? `
        <section class="terms-alpha-list">
          ${renderTermRows()}
        </section>
      ` : `
        <section class="terms-empty-state">
          <div class="terms-empty-icon" aria-hidden="true"></div>
          <p>添加自定义术语以提高转写质量</p>
        </section>
      `}
    </main>
  `;
  return `${base}${renderTermEditorSheet()}`;
}

function renderNotifications() {
  const items = [
    ["进度与账号提醒", "在录音上传、转写和总结完成，或账号需要操作时，收到提醒", "transcription"],
    ["设备状态提醒", "接收设备相关提醒，例如电量过低、电量充满或固件更新等", "device"],
    ["产品更新与优惠", "获取新功能、产品更新和优惠活动等信息", "product"]
  ];
  return `
    <main class="me-simple-page full-list-page prefs-page">
      ${meTop("消息与通知", { back: "APP-ME-26" })}
      <section class="prefs-section">
        ${prefsRow("允许通知", { toggle: settingsState.notificationsAllowed, action: "toggle-notifications-allowed" })}
      </section>
      <section class="prefs-section">
        <h2>通知类型</h2>
        ${items.map(([label, note, key]) => prefsRow(label, {
          note,
          toggle: settingsState.notifications[key],
          action: "toggle-notification",
          notification: key,
          disabled: !settingsState.notificationsAllowed
        })).join("")}
      </section>
    </main>
  `;
...
function renderAppLock() {
  const durations = ["立即", "1分钟后", "5分钟后", "15分钟后"];
  return `
    <main class="me-simple-page app-lock-page prefs-page">
      ${meTop("应用锁", { back: "APP-ME-26" })}
      <section class="prefs-section">
        ${prefsRow("应用锁", { toggle: settingsState.appLockEnabled, action: "toggle-app-lock" })}
      </section>
      ${settingsState.appLockEnabled ? `
        <section class="prefs-section">
          <h2>自动锁定</h2>
          ${durations.map((duration) => prefsRow(duration, {
            check: settingsState.autoLockDuration === duration,
            action: "select-auto-lock-inline",
            valueKey: duration,
            arrow: false
          })).join("")}
        </section>
...
function renderAutoLockDuration() {
  const durations = ["立即", "1分钟后", "5分钟后", "15分钟后"];
  return `
    <main class="me-simple-page full-list-page app-lock-page">
      ${meTop("自动锁定时长", { back: "APP-ME-38" })}
      <section class="prefs-section">
        ${durations.map((duration) => prefsRow(duration, { check: settingsState.autoLockDuration === duration, action: "select-auto-lock", valueKey: duration })).join("")}
      </section>
    </main>
  `;
}

function renderFaceIdPermission() {
  return `
    ${renderAppLock()}
    <div class="scrim" data-go="APP-ME-38"></div>
    <section class="permission-sheet">
      <button class="sheet-close" data-go="APP-ME-38" aria-label="关闭">×</button>
      <h2>允许“AI录音卡”使用 Face ID?</h2>
      <p>Face ID 仅用于解锁本机应用，生物识别数据由系统管理，应用不会读取或保存。</p>
      <button class="dark" data-action="allow-face-id">允许</button>
      <button class="outline" data-go="APP-ME-38">暂不允许</button>
    </section>
  `;
}
...
function renderConnectionGuideSteps(nextLabel) {
  return `
    <div class="connection-guide-steps" aria-label="第 1 步，了解连接；第 2 步，${h(nextLabel)}">
      <span class="current"><b>1</b><em>了解连接</em></span>
      <i aria-hidden="true"></i>
      <span><b>2</b><em>${h(nextLabel)}</em></span>
    </div>
  `;
}

function renderConnectionGuideArt(alt) {
  return `<img class="connection-guide-art" src="connection-guide-art.png" alt="${h(alt)}">`;
}

function renderConnectionGuideTrust(copy) {
  return `<p class="connection-guide-trust">${h(copy)}</p>`;
}
...
function renderFirmwareHotspotContent() {
  return `
    ...
    <section class="firmware-sheet hotspot-sheet connection-guide-sheet firmware-connection-guide">
      ${renderConnectionGuideSteps("更新固件")}
      <h2>连接设备开启高速传输</h2>
      <p class="connection-guide-copy">手机会临时连接 AI Recorder A1 的高速传输网络，以更快传输固件包。继续后将先下载固件包；当系统弹窗出现时，请点<strong>「加入」</strong>。</p>
      ${renderConnectionGuideArt("AI Recorder A1 与手机建立本地高速连接的示意图")}
      ${renderConnectionGuideTrust("仅用于传输固件包，完成后恢复原网络")}
      <button class="primary-btn" data-go="APP-DEV-08">继续更新</button>
      <button class="secondary-btn connection-guide-cancel" data-go="APP-DEV-03">取消</button>
...
function renderFirmwareUpdatingContent() {
  const steps = [
    ["download", "下载固件包"],
    ["connect", "连接到 AI Recorder A1"],
    ["transfer", "传输固件包到 AI Recorder A1"],
    ["install", "安装固件包"]
  ];
  const activeIndex = Math.max(0, steps.findIndex(([id]) => id === firmwareUpdateStep));
  const reconnectVisible = firmwareUpdateTransport === "wifi"
    && firmwareUpdateStep === "connect"
    && !firmwareJoinPromptOpen;
  return `
    ...
    <section class="firmware-steps">
      ${steps.map(([id, label], index) => `<div class="firmware-step ${index < activeIndex ? "done" : ""} ${index === activeIndex ? "active" : ""}" data-firmware-step="${id}"><i></i><span>${label}</span></div>`).join("")}
      ${reconnectVisible
        ? `<button class="primary-btn" data-action="reopen-firmware-hotspot-prompt">重新连接</button>`
...
function renderFirmwareUpdating() {
  ...
  return screen(renderFirmwareUpdatingContent(), { title: " ", back: "APP-DEV-03", bottom: false, compact: true });
}
...
function renderFirmwareConnectionFailed() {
  return screen(`
    ...
    <section class="firmware-sheet">
      ...
      <button class="primary-btn" data-action="retry-firmware-update">重试</button>
      <button class="secondary-btn" data-action="start-firmware-update" data-value="bluetooth">改用蓝牙更新</button>
    </section>
...
function renderHotspotTransferContent(state) {
  ...
  return `
    ...
    <section class="firmware-sheet sync-transfer-sheet hotspot-transfer-sheet ${isAwaiting ? "connection-guide-sheet" : `hotspot-connecting-sheet ${isFailed ? "is-failed" : ""}`}"${isAwaiting ? "" : ' role="status" aria-live="polite"'}>
      ${isAwaiting ? `
        ${renderConnectionGuideSteps("系统确认")}
        <h2>连接设备开启高速传输</h2>
        <p class="connection-guide-copy">手机会临时连接 AI Recorder A1 的高速传输网络。下一步系统弹窗出现后，请点<strong>「加入」</strong>。</p>
        ${renderConnectionGuideArt("AI Recorder A1 与手机建立本地高速连接的示意图")}
        ${renderConnectionGuideTrust("仅用于传输录音文件，完成后恢复原网络")}
      ...
      ` : `
        ...
        <h2>${isFailed ? "连接失败" : "连接中"}</h2>
        <div class="hotspot-conflict-tip">
          <i aria-hidden="true">i</i>
          <p>若已开启个人热点，你将无法连接到该设备的 Wi-Fi。请关闭后重试。</p>
        </div>
        <ol class="hotspot-connection-steps" aria-label="录音卡热点连接进度">
          <li class="done">
            <span class="hotspot-step-icon wifi-device" aria-hidden="true"></span>
            <b>AI Recorder A1 的 Wi-Fi 已打开</b>
            <em class="hotspot-step-result" aria-label="已完成">✓</em>
          </li>
          <li class="${isFailed ? "failed" : "loading"}">
            <span class="hotspot-step-icon wifi-signal" aria-hidden="true"></span>
            <b>连接到 AI Recorder A1 的热点</b>
            ${isFailed
              ? '<em class="hotspot-step-result failed" aria-label="连接失败">!</em>'
              : '<em class="hotspot-step-loading" aria-label="连接中"><i></i><i></i><i></i></em>'}
          </li>
        </ol>
        ${isFailed
          ? '<button class="primary-btn hotspot-retry-btn" data-action="retry-hotspot-connecting">重试</button>'
          : ''}
...
const renderers = {
  ...
  "APP-FILE-24": () => renderExportSheet("audio"),
  "APP-FILE-25": () => renderExportSheet("transcript"),
  "APP-FILE-26": () => renderExportSheet("marks"),
  "APP-FILE-27": () => renderExportSheet("summary"),
  "APP-FILE-28": () => renderExportSheet("mindmap"),
  ...
  "APP-AI-02": renderAudioLanguageSheet,
  "APP-AI-03": renderAiModelSheet,
  "APP-ME-02": renderUsage,
  "APP-ME-16": renderAccountSettingsHome,
  "APP-ME-26": renderPreferences,
  "APP-ME-27": renderTranscriptionLanguage,
  "APP-ME-28": renderSpeakerSettings,
  "APP-ME-29": renderVoiceSamples,
  "APP-ME-30": renderSpeakerName,
  "APP-ME-31": renderMicrophonePermission,
  "APP-ME-32": renderVoiceSampleRecording,
  "APP-ME-33": renderVoiceSampleComplete,
  "APP-ME-34": renderTerms,
  "APP-ME-35": renderIndustryList,
  "APP-ME-36": renderCommonTerms,
  "APP-ME-37": renderNotifications,
  "APP-ME-38": renderAppLock,
  "APP-ME-39": renderAutoLockDuration,
...
function renderFlow() {
  const flow = ["APP-PRD-00", "APP-ONB-01", "APP-ONB-02", "APP-ONB-03", "APP-ONB-05", "APP-ONB-07", "APP-ONB-06", "APP-ONB-04", "APP-DEV-01", "APP-DEV-02", "APP-DEV-04", "APP-DEV-05", "APP-HOME-01", "APP-SYNC-01", "APP-FILE-13", "APP-FILE-02", "APP-FILE-14", "APP-FILE-11", "APP-FILE-12", "APP-FILE-03", "APP-FILE-01", "APP-FILE-10", "APP-AI-01", "APP-REC-03", "APP-AI-02", "APP-AI-03", "APP-FILE-15", "APP-FILE-04", "APP-FILE-05", "APP-FILE-06", "APP-FILE-24", "APP-FILE-25", "APP-FILE-26", "APP-FILE-27", "APP-FILE-28", "APP-FILE-08", "APP-FILE-20", "APP-FILE-09", "APP-FILE-21", "APP-FILE-22", "APP-FILE-07", "APP-FILE-16", "APP-FILE-17", "APP-FILE-18", "APP-FILE-19", "APP-HOME-03", "APP-REC-02", "APP-REC-04", "APP-ME-02", "APP-ME-16", "APP-ME-26", "APP-ME-27", "APP-ME-28", "APP-ME-29", "APP-ME-30", "APP-ME-31", "APP-ME-32", "APP-ME-33", "APP-ME-34", "APP-ME-35", "APP-ME-36", "APP-ME-37", "APP-ME-38", "APP-ME-40", "APP-ME-39", "APP-ME-17", "APP-ME-18", "APP-ME-41", "APP-ME-19", "APP-ME-20", "APP-ME-21", "APP-ME-22", "APP-ME-05", "APP-ME-06", "APP-ME-07", "APP-ME-13", "APP-ME-14", "APP-ME-08", "APP-ME-15", "APP-ME-09", "APP-ME-23", "APP-ME-10", "APP-ME-11", "APP-ME-12", "APP-DEV-03", "APP-DEV-11", "APP-DEV-10", "APP-ME-04", "APP-ME-24", "APP-ME-25", "APP-DEV-06", "APP-DEV-07", "APP-DEV-08", "APP-DEV-09"];
...
function renderSpec(page) {
  ...
  document.getElementById("specBody").innerHTML = `
    ...
    <section class="spec-section"><h3>交互规则</h3><ul>${page.walkthroughNote ? `<li class="walkthrough-note">${h(page.walkthroughNote)}</li>` : ""}${(page.criticalRules || []).map((item) => `<li class="spec-critical-rule">${h(item)}</li>`).join("")}${page.rules.map((item) => `<li>${h(item)}</li>`).join("")}</ul></section>
...
function handleSettingsAction(action, target) {
  if (action === "toggle-readability") {
    settingsState.improveReadability = !settingsState.improveReadability;
    render();
    return true;
  }
  if (action === "dismiss-settings-notice") {
    settingsNotice = "";
    render();
    return true;
  }
  if (action === "confirm-readability") {
    settingsState.improveReadability = true;
    settingsNotice = "";
    render();
    return true;
  }
  if (action === "toggle-improve-ai") {
    settingsState.improveAi = !settingsState.improveAi;
    render();
    return true;
  }
  if (action === "select-language") {
    settingsState.transcriptionLanguage = target.dataset.value;
    go("APP-ME-26");
    return true;
  }
  if (action === "toggle-speaker-diarization") {
    settingsState.speakerDiarization = !settingsState.speakerDiarization;
    render();
    return true;
  }
  ...
  if (action === "allow-microphone") {
    settingsState.microphoneAllowed = true;
    settingsState.voiceRecording = false;
    go("APP-ME-32");
    return true;
  }
  if (action === "toggle-voice-recording") {
    if (!settingsState.voiceRecording) {
      settingsState.voiceRecording = true;
      render();
    } else {
      settingsState.voiceRecording = false;
      settingsState.voiceSampleCreated = true;
      go("APP-ME-33");
    }
    return true;
  }
  if (action === "toggle-custom-terms") {
    if (settingsState.customTermsEnabled) {
      settingsState.customTermsEnabled = false;
    } else if (!settingsState.industry && customTerms.length === 0) {
      pendingTermsEnableNotice = true;
    } else {
      settingsState.customTermsEnabled = true;
    }
    render();
    return true;
  }
  if (action === "dismiss-terms-enable-notice") {
    pendingTermsEnableNotice = false;
    render();
    return true;
  }
  if (action === "open-term-sheet") {
    termEditorIndex = null;
    termSheetMode = "add";
    termSheetError = "";
    render();
  ...
  if (action === "open-edit-term") {
    termEditorIndex = Number(target.dataset.termIndex);
    termSheetMode = "edit";
    termSheetError = "";
    render();
    return true;
  }
  if (action === "close-term-sheet") {
    termSheetMode = "";
    termEditorIndex = null;
    termSheetError = "";
    render();
  ...
  if (action === "clear-term-draft") {
    const input = document.getElementById("termSheetInput");
    if (input) {
      input.value = "";
      input.focus();
    }
  ...
  if (action === "delete-term-sheet") {
    if (termEditorIndex !== null) customTerms.splice(termEditorIndex, 1);
    termEditorIndex = null;
    termSheetMode = "";
    termSheetError = "";
    if (!settingsState.industry && customTerms.length === 0) settingsState.customTermsEnabled = false;
    render();
    return true;
  }
  if (action === "select-industry") {
    settingsState.industry = target.dataset.value || "";
    if (!settingsState.industry && customTerms.length === 0) settingsState.customTermsEnabled = false;
    pendingTermsEnableNotice = false;
    go("APP-ME-34");
    return true;
  }
  if (action === "save-term-sheet") {
    const value = document.getElementById("termSheetInput")?.value.trim() || "";
    if (!value) {
      termSheetError = "请输入术语";
      render();
      return true;
    }
    if (termSheetMode === "edit" && termEditorIndex !== null) {
      customTerms[termEditorIndex] = value;
    } else {
      if (!customTerms.includes(value)) customTerms.push(value);
    }
    termEditorIndex = null;
    termSheetMode = "";
    termSheetError = "";
  ...
  if (action === "toggle-notification") {
    ...
    if (key in settingsState.notifications) settingsState.notifications[key] = !settingsState.notifications[key];
    render();
    return true;
  }
  if (action === "toggle-notifications-allowed") {
    settingsState.notificationsAllowed = !settingsState.notificationsAllowed;
    render();
    return true;
  }
  ...
  if (action === "select-auto-lock-inline") {
    settingsState.autoLockDuration = target.dataset.value;
    render();
    return true;
  }
  if (action === "allow-face-id") {
    settingsState.faceIdAllowed = true;
    settingsState.appLockEnabled = true;
    go("APP-ME-38");
    return true;
  }
  if (action === "select-auto-lock") {
    settingsState.autoLockDuration = target.dataset.value;
    go("APP-ME-38");
    return true;
  }
...
function render() {
  ...
  const heatmapScroller = document.querySelector(".settings-week-heatmap-scroll");
  if (heatmapScroller) {
    requestAnimationFrame(() => {
...
document.addEventListener("click", (event) => {
  ...
  if (action && action.dataset.action === "select-record-mode") {
    recordMode = action.dataset.value === "manual" ? "manual" : "auto";
    render();
    return;
  }
  if (action && action.dataset.action === "select-record-scene") {
    selectedRecordScene = action.dataset.value || "auto";
    render();
    return;
  }
  if (action && action.dataset.action === "open-template-library") {
    generationNotice = "";
    templateReturnPage = "APP-AI-01";
    draftTemplateId = selectedTemplateId;
    go("APP-REC-03");
    return;
  }
  if (action && action.dataset.action === "close-template-library") {
    draftTemplateId = selectedTemplateId;
    go(templateReturnPage);
    return;
  }
  if (action && action.dataset.action === "select-template") {
    draftTemplateId = action.dataset.value || "";
    render();
    return;
  }
  if (action && action.dataset.action === "apply-template") {
  ...
  if (action && action.dataset.action === "submit-generation") {
    if (generationMode === "custom" && !selectedTemplateId) {
      generationNotice = "请选择模板";
      render();
      window.setTimeout(() => {
        if (currentPageId === "APP-AI-01" && generationNotice === "请选择模板") {
          generationNotice = "";
          render();
        }
      }, 1800);
  ...
  if (action && action.dataset.action === "toggle-generation-speakers") {
    generationSpeakerDiarization = !generationSpeakerDiarization;
    render();
    return;
  }
  if (action && action.dataset.action === "select-ai-language") {
    selectedAudioLanguage = action.dataset.value || "自动";
    go("APP-AI-01");
    return;
  }
  if (action && action.dataset.action === "select-ai-model") {
    selectedAiModel = action.dataset.value || "自动";
    go("APP-AI-01");
    return;
  }
  ...
  if (action && action.dataset.action === "start-firmware-update") {
    firmwareUpdateTransport = action.dataset.value === "bluetooth" ? "bluetooth" : "wifi";
    go("APP-DEV-08");
    return;
  }
  if (action && action.dataset.action === "retry-firmware-update") {
    firmwareRetryAtConnection = true;
    go("APP-DEV-08");
    return;
  }
  if (action && action.dataset.action === "reopen-firmware-hotspot-prompt") {
    if (currentPageId !== "APP-DEV-08" || firmwareUpdateStep !== "connect") return;
    firmwareJoinPromptOpen = true;
    render();
    return;
  }
  ...
  if (action && action.dataset.action === "retry-hotspot-connecting") {
    if (syncPrototypeTimer) {
      window.clearTimeout(syncPrototypeTimer);
      syncPrototypeTimer = null;
    }
    ...
    syncPrototypeTimer = window.setTimeout(() => {
      syncPrototypeTimer = null;
      if (currentPageId !== "APP-SYNC-01" || hotspotTransferState !== "connecting") return;
  ...
  if (action && action.dataset.action === "ask-hide-tip") {
    askPlaudTipVisible = false;
    render();
    return;
  }
  ...
  if (action && action.dataset.action === "file-detail-set-tab") {
    persistFileDetailMarkerEdit();
    persistFileDetailContentEdit();
    const allowed = currentPageId === "APP-FILE-01" ? ["markers", "summary", "transcript"] : ["markers", "summary"];
    fileDetailActiveTab = allowed.includes(action.dataset.value) ? action.dataset.value : allowed[0];
    render();
    return;
  }
  if (action && action.dataset.action === "file-detail-set-summary-tab") {
    persistFileDetailContentEdit();
    const target = action.dataset.value || "summary";
    fileDetailActiveTab = "summary";
    scrollFileDetailSummaryToSection(target);
    return;
  }
  if (action && action.dataset.action === "file-detail-open-format-panel") {
    fileDetailFormatPanel = action.dataset.format || "main";
    rerenderFileDetailFormatEditor();
    return;
  }
  if (action && action.dataset.action === "file-detail-format-back") {
    fileDetailFormatPanel = "main";
    rerenderFileDetailFormatEditor();
    return;
  }
  if (action && action.dataset.action === "file-detail-set-block") {
    fileDetailFormatState.block = action.dataset.format || "body";
    storeFileDetailFormatState();
    rerenderFileDetailFormatEditor();
    return;
  }
  if (action && action.dataset.action === "file-detail-toggle-inline") {
    const format = action.dataset.format || "";
    if (["bold", "italic", "underline", "strike", "code"].includes(format)) fileDetailFormatState[format] = !fileDetailFormatState[format];
    storeFileDetailFormatState();
    rerenderFileDetailFormatEditor();
    return;
  }
  if (action && action.dataset.action === "file-detail-set-list") {
    const list = action.dataset.format || "none";
    fileDetailFormatState.list = fileDetailFormatState.list === list ? "none" : list;
    if (fileDetailFormatState.list === "none") fileDetailFormatState.indent = 0;
    storeFileDetailFormatState();
    rerenderFileDetailFormatEditor();
    return;
  }
  if (action && action.dataset.action === "file-detail-format") {
    const format = action.dataset.format || "";
    if (format === "outdent") fileDetailFormatState.indent = Math.max(0, fileDetailFormatState.indent - 1);
    if (format === "indent") fileDetailFormatState.indent = Math.min(2, fileDetailFormatState.indent + 1);
    if (format === "quote") fileDetailFormatState.quote = !fileDetailFormatState.quote;
    if (format === "undo" && fileDetailEditHistory.length > 1) {
      const current = fileDetailEditHistory.pop();
      fileDetailEditRedo.push(current);
      fileDetailContentEditor.draft = fileDetailEditHistory.at(-1) || "";
    }
    if (format === "redo" && fileDetailEditRedo.length) {
      const next = fileDetailEditRedo.pop();
      fileDetailEditHistory.push(next);
      fileDetailContentEditor.draft = next;
    }
    storeFileDetailFormatState();
    rerenderFileDetailFormatEditor();
    return;
  }
  if (action && action.dataset.action === "file-detail-hide-keyboard") {
    persistFileDetailMarkerEdit();
    persistFileDetailContentEdit();
    render();
    return;
  }
  if (action && action.dataset.action === "file-detail-keyboard-key") {
    const input = activeFileDetailInput();
    if (!input) return;
    const key = action.dataset.key || "";
    if (key === "⌫") input.value = input.value.slice(0, -1);
    else if (key === "换行") input.value += "\n";
    else if (key === "空格") input.value += " ";
    else if (["ABC", "DEF", "GHI", "JKL", "MNO", "PQRS", "TUV", "WXYZ", ",", "。", "?", "@#"].includes(key)) input.value += key;
    else {
      showFileDetailNotice("原型键盘保持中文输入");
      focusFileDetailEditor(input.id);
      return;
    }
  ...
  if (action && action.dataset.action === "file-detail-toggle-play") {
    fileDetailAudioPlaying = !fileDetailAudioPlaying;
    render();
    return;
  }
  if (action && action.dataset.action === "file-detail-skip") {
    fileDetailAudioSeconds = Math.max(0, fileDetailAudioSeconds + Number(action.dataset.seconds || 0));
    render();
    return;
  }
  if (action && action.dataset.action === "file-detail-seek-track") {
    const rect = action.getBoundingClientRect();
    const ratio = Math.min(1, Math.max(0, (event.clientX - rect.left) / Math.max(1, rect.width)));
    fileDetailAudioSeconds = Math.round(Number(action.dataset.duration || 0) * ratio);
    render();
    return;
  }
  if (action && action.dataset.action === "file-detail-seek-marker") {
    const mark = recordingMarks.find((item) => item.id === action.dataset.markerId);
    if (!mark) return;
    fileDetailAudioSeconds = fileDetailTimeToSeconds(mark.time);
    fileDetailAudioPlaying = true;
    showFileDetailNotice(`已定位到 ${mark.time}`);
    render();
    return;
  }
  if (action && action.dataset.action === "file-detail-edit-marker") {
    const mark = recordingMarks.find((item) => item.id === action.dataset.markerId);
    if (!mark) return;
    fileDetailMarkerEditor = {
      type: mark.type,
      markId: mark.id,
      time: mark.time,
      source: mark.source,
      draft: mark.type === "photo" ? (mark.caption || (currentPageId === "APP-FILE-01" ? "白板记录了项目交付时间" : "")) : (mark.text || "")
    };
    render();
    focusFileDetailEditor("fileDetailMarkerInput");
    return;
  }
  if (action && action.dataset.action === "file-detail-edit-content") {
    const key = action.dataset.key || "";
    const kind = action.dataset.kind || "";
    let draft = "";
    if (kind === "summary") draft = fileDetailContent.summary;
    if (kind === "transcript") {
      const index = Number(key.split("-")[1]);
      draft = fileDetailContent.transcript[index]?.text || "";
    }
    fileDetailFormatPanel = "main";
    fileDetailFormatState = { ...defaultFileDetailFormatState(), ...(fileDetailContentFormats[key] || {}) };
    fileDetailEditHistory = [draft];
    fileDetailEditRedo = [];
    fileDetailContentEditor = { key, kind, draft };
    if (currentPageId === "APP-FILE-01") {
      fileDetailActiveTab = kind;
      if (kind === "summary") fileDetailSummaryTab = "summary";
    }
    render();
    focusFileDetailEditor("fileDetailContentInput");
    return;
  }
  if (action && action.dataset.action === "file-detail-open-photo") {
    fileDetailPhotoViewerId = action.dataset.markerId || "";
    render();
    return;
  }
  if (action && action.dataset.action === "file-detail-close-photo") {
    fileDetailPhotoViewerId = "";
    render();
    return;
  }
  if (action && action.dataset.action === "file-detail-delete-photo") {
    const markerId = action.dataset.markerId || "";
    recordingMarks = recordingMarks.filter((mark) => mark.id !== markerId);
    if (fileDetailPhotoViewerId === markerId) fileDetailPhotoViewerId = "";
    showFileDetailNotice("照片标记已删除");
    render();
    return;
  }
  if (action && action.dataset.action === "open-sn-tip") {
    deviceSnTipOpen = true;
    render();
    return;
  }
  if (action && action.dataset.action === "close-sn-tip") {
    deviceSnTipOpen = false;
    render();
    return;
  }
  ...
  if (action && action.dataset.action === "reset-recording-demo") {
    recordingMarks = recordingBaseMarks.map((mark) => ({ ...mark }));
    recordingEditor = null;
    recordingCameraOpen = false;
    recordingPhotoViewerId = "";
    recordingCameraPrompt = false;
    recordingExpandedMarkIds = new Set();
    recordingNotice = "";
    render();
    return;
  }
  if (action && action.dataset.action === "start-recording-input") {
    openRecordingEditor("new-input");
    render();
    requestAnimationFrame(() => document.getElementById("recordingEditorInput")?.focus());
    return;
  }
  if (action && action.dataset.action === "open-recording-camera") {
    recordingEditor = null;
    recordingPhotoViewerId = "";
    recordingCameraOpen = true;
    render();
  ...
  if (action && action.dataset.action === "close-recording-camera") {
    recordingCameraOpen = false;
    render();
    return;
  }
  if (action && action.dataset.action === "capture-recording-photo") {
    captureRecordingPhoto();
    render();
    return;
  }
  if (action && action.dataset.action === "open-recording-photo") {
    recordingPhotoViewerId = action.dataset.markerId || "";
    render();
    return;
  }
  if (action && action.dataset.action === "close-recording-photo") {
    recordingPhotoViewerId = "";
    render();
    return;
  }
  if (action && action.dataset.action === "delete-recording-photo") {
    const markerId = action.dataset.markerId || "";
    recordingMarks = recordingMarks.filter((mark) => mark.id !== markerId);
    if (recordingPhotoViewerId === markerId) recordingPhotoViewerId = "";
    showRecordingNotice("照片标记已删除");
    render();
    return;
  }
  if (action && action.dataset.action === "edit-recording-photo-caption") {
    openRecordingEditor("photo-caption", action.dataset.markerId || "");
    render();
    requestAnimationFrame(() => document.getElementById("recordingEditorInput")?.focus());
    return;
  }
  if (action && action.dataset.action === "edit-recording-mark") {
    openRecordingEditor("edit-text", action.dataset.markerId || "");
    render();
    requestAnimationFrame(() => document.getElementById("recordingEditorInput")?.focus());
    return;
  }
  if (action && action.dataset.action === "toggle-recording-mark-expanded") {
    const markerId = action.dataset.markerId || "";
    if (recordingExpandedMarkIds.has(markerId)) recordingExpandedMarkIds.delete(markerId);
    else recordingExpandedMarkIds.add(markerId);
    render();
    return;
  }
  if (action && action.dataset.action === "cancel-recording-editor") {
    closeRecordingEditor();
    render();
    return;
  }
  if (action && action.dataset.action === "complete-recording-editor") {
    completeRecordingEditor();
    render();
    if (recordingEditor) requestAnimationFrame(() => document.getElementById("recordingEditorInput")?.focus());
    return;
  }
  if (action && action.dataset.action === "delete-recording-mark-text") {
    if (recordingEditor?.markId) {
      recordingMarks = recordingMarks.map((mark) => mark.id === recordingEditor.markId ? { ...mark, text: "" } : mark);
      closeRecordingEditor();
      showRecordingNotice("标记文字已删除，时间锚点已保留");
    }
    render();
    return;
  }
  if (action && action.dataset.action === "recording-keyboard-key") {
    if (!recordingEditor) return;
    const key = action.dataset.key || "";
    if (key === "switch") {
      showRecordingNotice("原型键盘保持中文输入");
    } else if (key === "backspace") {
      recordingEditor.draft = recordingEditor.draft.slice(0, -1);
    } else if (key === "space") {
      recordingEditor.draft += " ";
    } else {
      recordingEditor.draft += key;
    }
    render();
    requestAnimationFrame(() => {
      const input = document.getElementById("recordingEditorInput");
      input?.focus();
      input?.setSelectionRange(input.value.length, input.value.length);
    });
    return;
  }
  if (action && action.dataset.action === "simulate-recording-photo-ai") {
    simulateRecordingPhotoAiCaption();
    render();
    return;
  }
  ...
  if (action && action.dataset.action === "home-complete-sync") {
    completeCurrentHomeSyncTask();
    render();
    return;
  }
  if (action && action.dataset.action === "home-start-single-sync-batch") {
    startSingleHomeSyncBatch();
    render();
    return;
  }
  if (action && action.dataset.action === "home-reset-sync-batch") {
    resetMultiHomeSyncBatch();
    render();
    return;
  }
  if (action && action.dataset.action === "home-show-no-sync-tasks") {
    showNoHomeSyncTasks();
    render();
    return;
  }
  if (action && action.dataset.action === "home-show-no-device") {
    showNoHomeDevice();
    render();
    return;
  }
  ...
  if (action && action.dataset.action === "retry-home-failed-file") {
    homeFailedFileState = "retrying";
    render();
    return;
  }
  if (action && action.dataset.action === "open-home-priority-menu") {
    openHomePriorityMenu(action.dataset.taskId || "", event.detail === 0);
    return;
  }
  if (action && action.dataset.action === "close-home-priority-menu") {
    closeHomePriorityMenu();
    render();
    return;
  }
  if (action && action.dataset.action === "confirm-home-priority") {
    if (setHomePriorityTask(action.dataset.taskId || "")) render();
    return;
  }
  ...
  if (action && action.dataset.action === "show-recycle-list") {
    recycleBinItems = defaultRecycleBinItems.map((item) => ({ ...item }));
    recycleSelectionMode = false;
    selectedRecycleIds = new Set();
    recycleDeleteConfirmOpen = false;
    recycleNotice = "";
    if (currentPageId !== "APP-FILE-23") go("APP-FILE-23");
    else render();
    return;
  }
  if (action && action.dataset.action === "show-recycle-empty") {
    recycleBinItems = [];
    recycleSelectionMode = false;
    selectedRecycleIds = new Set();
    recycleDeleteConfirmOpen = false;
  ...
  if (action && action.dataset.action === "start-recycle-selection") {
  ...
  if (action && action.dataset.action === "finish-recycle-selection") {
    recycleSelectionMode = false;
    selectedRecycleIds = new Set();
    recycleDeleteConfirmOpen = false;
    render();
  ...
  if (action && action.dataset.action === "toggle-recycle-item") {
    const itemId = action.dataset.recycleId || "";
    if (!recycleBinItems.some((item) => item.id === itemId)) return;
    recycleSelectionMode = true;
    const nextSelection = new Set(selectedRecycleIds);
    if (nextSelection.has(itemId)) nextSelection.delete(itemId);
    else nextSelection.add(itemId);
    selectedRecycleIds = nextSelection;
    recycleNotice = "";
    render();
    return;
  }
  if (action && action.dataset.action === "restore-recycle-items") {
    const count = selectedRecycleIds.size;
    if (!count) return;
    recycleBinItems = recycleBinItems.filter((item) => !selectedRecycleIds.has(item.id));
    selectedRecycleIds = new Set();
    recycleSelectionMode = false;
    recycleNotice = `已恢复 ${count} 个文件`;
    render();
    return;
  }
  if (action && action.dataset.action === "request-recycle-delete") {
    if (!selectedRecycleIds.size) return;
    recycleDeleteConfirmOpen = true;
    render();
    return;
  }
  if (action && action.dataset.action === "cancel-recycle-delete") {
    recycleDeleteConfirmOpen = false;
    render();
    return;
  }
  if (action && action.dataset.action === "confirm-recycle-delete") {
    const count = selectedRecycleIds.size;
    if (!count) return;
    recycleBinItems = recycleBinItems.filter((item) => !selectedRecycleIds.has(item.id));
    selectedRecycleIds = new Set();
  ...
  if (action && action.dataset.action === "close-export-sheet") {
    closeExportSheet();
    return;
  }
  if (action && action.dataset.action === "toggle-export-format-menu") {
    const type = action.dataset.exportType;
    if (!exportSheetConfigs[type]) return;
    activeExportFormatMenu = activeExportFormatMenu === type ? "" : type;
    exportNotice = "";
    render();
    return;
  }
  if (action && action.dataset.action === "select-export-format") {
    const type = action.dataset.exportType;
    const format = action.dataset.exportFormat;
    if (!exportSheetConfigs[type]?.formats.includes(format)) return;
    exportSheetState[type] = { ...exportSheetState[type], format };
    activeExportFormatMenu = "";
    exportNotice = "";
    render();
    return;
  }
  if (action && action.dataset.action === "toggle-export-option") {
    const type = action.dataset.exportType;
    const option = action.dataset.exportOption;
    if (!Object.prototype.hasOwnProperty.call(exportSheetState[type] || {}, option)) return;
    exportSheetState[type] = { ...exportSheetState[type], [option]: !exportSheetState[type][option] };
    exportNotice = "";
    render();
    return;
  }
  if (action && action.dataset.action === "submit-export") {
    const type = action.dataset.exportType;
    const config = exportSheetConfigs[type];
    if (!config) return;
    activeExportFormatMenu = "";
    exportNotice = `已生成${config.title}，可通过系统分享保存或发送`;
    render();
    return;
  }
...
document.addEventListener("pointerdown", (event) => {
  const editDockButton = event.target.closest(".file-detail-edit-dock button");
  if (editDockButton) {
    fileDetailDockInteraction = true;
    window.setTimeout(() => { fileDetailDockInteraction = false; }, 80);
    event.preventDefault();
    return;
  }
  const priorityTrigger = event.target.closest("[data-action='open-home-priority-menu']");
  if (!priorityTrigger) return;
  clearHomePriorityLongPressTimer();
  const taskId = priorityTrigger.dataset.taskId || "";
  homePriorityLongPressTimer = window.setTimeout(() => {
    homePriorityLongPressTimer = null;
    openHomePriorityMenu(taskId);
  }, 500);
});

["pointerup", "pointercancel", "pointermove"].forEach((eventName) => {
  document.addEventListener(eventName, clearHomePriorityLongPressTimer);
});
...
document.addEventListener("contextmenu", (event) => {
  ...
  event.preventDefault();
  clearHomePriorityLongPressTimer();
  openHomePriorityMenu(priorityTrigger.dataset.taskId || "");
});

document.addEventListener("keydown", (event) => {
  const priorityTrigger = event.target.closest?.("[data-action='open-home-priority-menu']");
  if (priorityTrigger && ["Enter", " ", "Spacebar"].includes(event.key)) {
    event.preventDefault();
    openHomePriorityMenu(priorityTrigger.dataset.taskId || "", true);
    return;
  }
  if (event.key === "Escape" && currentPageId === "APP-REC-04") {
    if (recordingPhotoViewerId) recordingPhotoViewerId = "";
    else if (recordingCameraOpen) recordingCameraOpen = false;
    else if (recordingEditor) recordingEditor = null;
    else return;
    render();
    return;
  }
    if (event.key === "Escape" && ["APP-FILE-01", "APP-FILE-10", "APP-FILE-15"].includes(currentPageId)) {
    if (fileDetailPhotoViewerId) fileDetailPhotoViewerId = "";
    else if (fileDetailMarkerEditor) persistFileDetailMarkerEdit();
    else if (fileDetailContentEditor) persistFileDetailContentEdit();
    else return;
    render();
    return;
  }
  if (event.key !== "Escape" || !homePriorityMenuTaskId) return;
  const taskId = homePriorityMenuTaskId;
  closeHomePriorityMenu();
  render();
  requestAnimationFrame(() => {
    document.querySelector(`[data-action="open-home-priority-menu"][data-task-id="${taskId}"]`)?.focus();
  });
});

document.addEventListener("keydown", (event) => {
  if (event.target.id !== "searchInput" || event.key !== "Enter") return;
  event.preventDefault();
  submitSearchDraft();
});
...
document.addEventListener("input", (event) => {
  ...
  if (event.target.id === "fileDetailContentInput" && fileDetailContentEditor) {
    fileDetailContentEditor.draft = event.target.value;
    pushFileDetailEditHistory(event.target.value);
    event.target.style.height = "auto";
    event.target.style.height = `${event.target.scrollHeight}px`;
  }
  if (event.target.id === "recordingEditorInput" && recordingEditor) {
    recordingEditor.draft = event.target.value;
    const completeButton = document.querySelector("[data-action='complete-recording-editor']");
    const canComplete = recordingEditor.mode === "photo-caption" || Boolean(recordingEditor.draft.trim());
...
document.addEventListener("focusout", (event) => {
  if (event.target.id === "fileDetailMarkerInput" && fileDetailMarkerEditor) {
    persistFileDetailMarkerEdit();
    window.setTimeout(() => {
      if (["APP-FILE-01", "APP-FILE-10", "APP-FILE-15"].includes(currentPageId) && !fileDetailMarkerEditor) render();
    }, 0);
    return;
  }
  if (event.target.id === "fileDetailContentInput" && fileDetailContentEditor) {
    if (fileDetailDockInteraction || event.relatedTarget?.closest?.(".file-detail-edit-dock")) return;
    persistFileDetailContentEdit();
    window.setTimeout(() => {
      if (["APP-FILE-01", "APP-FILE-10", "APP-FILE-15"].includes(currentPageId) && !fileDetailContentEditor) render();
    }, 0);
  }
});
