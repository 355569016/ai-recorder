...
const pages = [
  ...
  },
  {
    ...
    rules: ["进入页面后直接展示模板分类列表，不展示我的模板与探索标签栏。", "模板按通用、会议、演讲、通话、面试/采访等分类展示。", "未选择模板时由系统自动匹配。", "选择模板后需点击使用此模板才写回转写与生成页面。"],
    states: ["未选择", "已选择", "模板加载失败"],
    deps: ["AI 模板服务或本地模板配置，接口待后端定义。"],
    acceptance: ["进入页面即展示按分类组织的模板列表。", "页面不出现我的模板与探索标签栏。", "选中模板有清晰的选中态并可返回转写与生成页面。", "模板失败时保持自动匹配降级。"]
  },
  {
    id: "APP-REC-04",
    group: "录音",
  ...
  },
  {
    ...
    fields: [["generation_mode", "生成方式", "用户选择"], ["template_id", "生成模板", "系统匹配/用户选择"], ["estimated_minutes", "预计消耗", "用量服务"], ["remaining_minutes", "剩余额度", "用量服务"], ["speaker_diarization_enabled", "自动标注说话人", "用户选择"], ["audio_language", "音频语言", "用户选择"], ["ai_model", "AI 模型", "用户选择"]],
    rules: ["文件列表和文件详情为空的生成入口必须先进入本页。", "点击立即生成后进入文件详情生成中页面。", "自动生成自动匹配转写模板与总结。", "自定义生成可调整模板、说话人、语言和 AI 模型。", "额度不足时立即生成按钮置灰并禁止提交。", "弹层打开时底层文件详情整体进入同一蒙版状态，包括吸顶的大纲、智能总结、转写记录等导航。", "页面不重复展示录音场景，也不在按钮上方展示云端上传说明。"],
    states: ["自动生成", "自定义生成", "额度充足", "额度不足", "模板已选择", "语言选择", "模型选择"],
    deps: ["用量服务：预估分钟、余额。", "会员服务：订阅等级与权益。", "支付服务：购买分钟数、升级订阅。", "AI 服务：模板列表、模型列表。", "云端文件：上传凭证。"],
    acceptance: ["弹层展示自动与自定义生成、模板配置和用量确认。", "底层大纲导航不得穿透蒙版或保持高亮。", "选择模板进入无标签栏的分类模板列表。", "额度充足时立即生成可提交并进入文件详情生成中。"]
  },
  {
    id: "APP-AI-02",
    group: "AI",
...
function selectedTemplateLabel() {
  return templateById(selectedTemplateId)?.title || "自动匹配";
}
...
function renderGenerationSettingRow(label, value, opts = {}) {
  ...
  const tail = opts.toggle
    ...
    : `<span class="generation-setting-value">${h(value)}<i>›</i></span>`;
  return `<button class="generation-setting-row"${attrs}><strong>${h(label)}</strong>${tail}</button>`;
}
  ...
  return `
    ...
    <section class="bottom-sheet generate-sheet">
      ...
      <button class="option ${generationMode === "auto" ? "selected" : ""}" data-mode="auto">
        ...
        <div><strong>自动生成</strong><p class="caption">自动匹配转写模板与总结，无需手动设置</p></div>
      </button>
      ...
      ${generationMode === "custom" ? `
        <div class="generation-settings">
          ${renderGenerationSettingRow("模板", `${selectedTemplateLabel()} · 查看全部`, { action: "open-template-library" })}
