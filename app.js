...
const pages = [
  ...
  },
  {
    ...
    rules: [
      ...
      "为控制原型页面信息密度，APP-HOME-01 默认只展示前 3 条等待传输文件示例；后台仍按完整待传输任务数量判断是否满足优先同步条件。",
      "等待传输状态只显示“等待传输”，不显示队列顺位、传输方式、已传输/总文件大小或单文件进度。",
      "当待传输任务超过 3 个时，每个等待传输文件在“等待传输”状态后显示“优先同步”操作；设为优先后，目标任务移动到当前传输任务之后，当前任务保持不变。待传输任务不超过 3 个时不显示该操作。",
      "全部文件中的正在传输和等待传输行整体置灰，且不显示右箭头或进入详情操作；正在传输行不可点击并可显示单文件百分比，等待传输行除条件满足时的“优先同步”操作外不可点击。",
      "同步传输失败后优先进入后台自动重试；设备恢复连接时自动续传和重试。未达到自动重试限额期间，文件维持置灰的正在重试状态，不显示失败状态或手动重试按钮；达到限额后才显示“同步失败，请重试…”和重试按钮，且不显示“录音已安全保留”等未经确认的保障文案。",
      ...
      "每个文件行右侧不显示进入详情的箭头；正常文件通过生成、详情等明确按钮进入下一步。",
    ...
    criticalRules: [
      ...
      "同步失败处理强规则：传输失败后先由系统自动重试，不立即向用户显示失败；设备重新连接后自动继续重试。只有达到自动重试限额后，文件才显示“同步失败，请重试…”和“重试”按钮。自动重试限额及退避间隔由同步重试策略配置。"
    ],
    states: ["未添加录音卡", "未绑定/未连接", "设备已连接", "当前无同步任务", "录音中持续同步", "单任务同步中", "多任务同步中", "文件正在传输且不可点击", "文件等待传输且不可点击", "文件列表优先同步", "同步失败自动重试", "设备重连后自动重试", "达到自动重试限额后失败可重试", "任务优先级已调整", "单个任务同步完成", "本批次同步完成", "新同步批次", "空文件", "加载更多"],
    ...
    acceptance: [
      ...
      "APP-HOME-01 原型默认只展示 3 条等待传输文件示例；当后台待传输任务总数超过 3 个时，这 3 条仍显示优先同步操作。",
      "正在传输和等待传输文件整体置灰，不显示右箭头、详情操作或单文件进度条；正在传输文件显示“正在传输… + 单文件百分比”，等待文件只显示“等待传输”。",
      "等待传输文件不显示队列顺位、传输方式或已传输/总文件大小。",
      "同步传输失败且未达到自动重试限额时，不显示失败状态或重试按钮；设备重新连接后必须自动继续重试。达到自动重试限额后，文件才显示“同步失败，请重试…”和重试按钮，不显示“录音已安全保留”；点击重试后该行切换为置灰的正在重试状态。",
      ...
      "待传输任务超过 3 个时，每个等待传输文件在状态后显示“优先同步”；操作后当前任务保持不变，目标任务移动到当前任务之后并显示“已优先”。待传输任务不超过 3 个时不显示该操作。",
      ...
      "正常文件通过生成、详情等明确按钮操作，所有文件行右侧不额外显示详情箭头。",
...
function fileCard(file) {
  ...
  const priorityAction = file.priorityAction
    ? `<button class="file-transfer-priority ${file.priorityActive ? "active" : ""}" data-action="prioritize-home-sync-task" data-task-id="${h(file.taskId)}"${file.priorityActive ? " disabled" : ""}>${file.priorityActive ? "已优先" : "优先同步"}</button>`
    : "";
  const statusNode = disabled
    ? `<div class="file-transfer-status"><span role="status" aria-live="polite">${h(file.statusText)}</span>${file.transferState === "syncing" ? '<em class="sync-state-spinner file-transfer-spinner" aria-hidden="true"></em>' : ""}${priorityAction}</div>`
    : file.errorText
      ? `<div class="file-card-recovery"><span>${h(file.errorText)}</span><button class="secondary-btn" ${file.actionKey ? `data-action="${h(file.actionKey)}"` : `data-go="${file.go}"`}>${h(file.action)}</button></div>`
  ...
  return `
    <div class="file-card ${file.featured ? "featured" : ""} ${disabled ? `file-card-transfer-disabled ${file.transferState}` : ""}"${disabled && !file.priorityAction ? ' aria-disabled="true"' : ""}>
      <div>
...
function renderHomeTransferFileCards() {
  ...
  const showPriority = pendingTasks.length > 3;
  const pendingMeta = [
    "今天 09:20 · 00:28:14",
    "今天 08:45 · 00:19:08",
    "昨天 18:10 · 00:35:12",
  ...
  return [
    ...
    ...pendingTasks.slice(0, 3).map((task, index) => fileCard({
      ...
      statusText: "等待传输",
      priorityAction: showPriority,
      priorityActive: homeSyncPriorityTaskId === task.id,
      taskId: task.id
    }))
  ].join("");
}
...
function prioritizeHomeSyncTask(taskId) {
  if (homeSyncQueue.filter((task) => task.status === "pending").length <= 3) return;
  const taskIndex = homeSyncQueue.findIndex((task) => task.id === taskId && task.status === "pending");
  if (taskIndex < 0) return;
  const [task] = homeSyncQueue.splice(taskIndex, 1);
  const activeIndex = homeSyncQueue.findIndex((item) => item.status === "syncing");
  homeSyncQueue.splice(activeIndex >= 0 ? activeIndex + 1 : 0, 0, task);
  homeSyncPriorityTaskId = task.id;
}
...
function completeCurrentHomeSyncTask() {
  ...
  if (homeSyncPriorityTaskId === activeTask.id) homeSyncPriorityTaskId = "";
...
function renderHomeContent({ showSync = true } = {}) {
  const savedRecordingCard = homeSyncStage === "saved" || homeSyncStage === "idle"
    ? fileCard({ featured: true, name: "周一项目复盘", meta: "刚刚 · 00:43:52", source: "硬件录音", state: "未转写", tone: "amber", action: "生成", go: "APP-AI-01" })
    : "";
  ...
  return `
    ...
    <section class="card file-list-panel" id="homeFileList">
      ...
      ${savedRecordingCard}
      ${homeFailedFileState === "failed"
        ? fileCard({ name: "产品讨论", meta: "昨天 16:40 · 00:58:20", source: "硬件录音", errorText: "同步失败，请重试…", action: "重试", actionKey: "retry-home-failed-file" })
...
document.addEventListener("click", (event) => {
  ...
  if (action && action.dataset.action === "prioritize-home-sync-task") {
    prioritizeHomeSyncTask(action.dataset.taskId || "");
    render();
    return;
  }
