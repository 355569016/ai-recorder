...
const pages = [
  ...
  },
  {
    ...
    rules: ["空状态保留标题、音频播放器和右上角操作。", "未生成状态以标记作为默认一级页签，完整展示录音期间的标记；录音结束后不允许新增标记，只能原位编辑已有文字或照片说明、查看照片和删除照片。", "总结页签显示可生成笔记提示，底部固定生成主按钮。", "生成按钮文案只显示生成；点击后进入生成方式与用量确认弹层，并在确认时冻结本次标记快照。"],
  ...
  },
  {
    ...
    acceptance: ["文件列表和文件详情为空点击生成都先进入生成方式与用量确认。", "生成中页结构与未生成页连续，默认打开总结并可切回标记维护。", "标记页明确说明本次生成已读取、后续修改用于下次生成。", "完成状态进入文件详情页，不再停留在生成确认页。"],
...
function fileCard(file) {
  ...
  const statusNode = disabled
    ? file.transferState === "pending" && file.taskId
      ? `<div class="file-transfer-status-wrap">
